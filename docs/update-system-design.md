...
2. **Sign locally**: extend `scripts/publish-repo-pages.sh` to call `tools/eos-repo-sign` and emit `repo.toml.sig` (hybrid ed25519+ML-DSA-65) — the tool exists but the publisher never runs it today (the R-503 "security theater as shipped" gap). This is the single most important wiring fix: it makes the manifest signature actually authenticate something at runtime.
