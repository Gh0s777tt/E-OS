...
2. **Sign and verify the manifest/catalog** — `repo.toml`/per-package tomls are currently unauthenticated client-side; wire `tools/eos-repo-sign` into `publish-repo-pages.sh` to emit `catalog.toml.sig` + `repo.toml.sig`, and add fetch+verify in `pkg-lib`.
