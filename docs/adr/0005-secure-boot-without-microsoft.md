---
title: ADR-0005: Secure Boot bez zależności od Microsoftu
status: accepted
...
- Klucz Secure Boot to **kolejny klucz operatora**, poza repozytorium (jak `eos-repo-sign` i
  minisign) — patrz `docs/reference/keys-and-tokens.md`.
...
**Dowiedzione end-to-end:** świeże live ISO pod QEMU z firmware Secure Boot startuje pełny system (E-OS Bootloader → RedoxFS) gdy firmware ufa naszemu kluczowi, i daje Access Denied z obcym. Bootloader **systemu zainstalowanego** (ta sama paczka) sygnowany jest tym samym mechanizmem — sygnowany jest tym samym mechanizmem i **również dowiedziony** (`U-208`): obraz `harddrive.img` bootuje pod firmware Secure Boot z naszym kluczem, Access Denied z obcym. OBA nośniki — instalacyjny i zainstalowany — są więc pokryte. Jedyne zastrzeżenie wdrożeniowe: podpis następuje przy świeżym `cook`, dlatego `scripts/eos-sb-setup-key.sh` unieważnia paczkę bootloadera przy kładzeniu klucza.
