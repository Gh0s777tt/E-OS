...
| migawki / subwoluminy dla użytkownika | **nie** | `docs/architecture/update-system.md`: *„no mature snapshot/subvolume primitive"*; `grep -rni snapshot docs/ recipes/ mk/ scripts/ config/` daje **28 trafień w 14 plikach**, i żadne nie jest prymitywem: propozycje (`ROADMAP.md` §21), plany (`update-system-design.md`, `system-updates.md`) oraz **trafienia bez związku** — aplikacja GNOME Snapshot (`recipes/wip/gnome/snapshot/recipe.toml:3`), adresy tarballi `git.zx2c4.com/*/snapshot/`, komentarze w `mk/fetch-sha256.txt:20`, `scripts/ci-integrity.sh:316`, `scripts/docs-pdf.sh:36` | **NOWY PODSYSTEM** |
