...
- ⏳ `R-D06` **Fix netsurf ET_EXEC crash on aarch64** — The only bundled browser dies at startup (ESR 0x92000047 L3 fault) because it's a non-PIE `ET_EXEC`, and aarch64-Redox requires a PIE. **Diagnosed (2026-07-19), and it's a build-infrastructure quagmire, not a code fix — hence L:** (1) the redox gcc *can* produce a PIE (`-fPIE -pie` verified: `ELF … pie executable`), and a CC-wrapper in the recipe that appends `-fPIE` to every compile and `-fPIE -pie` to every link cleanly forces it; BUT (2) the cook **downloads the upstream prebuilt `static.redox-os.org/pkg/aarch64-unknown-redox/netsurf.pkgar` (a non-PIE ET_EXEC) via `--repo-binary`**, so the recipe change is inert until a *source* build is forced (`repo clean netsurf` + `repo cook netsurf` without `--repo-binary`); AND (3) forcing that source build cascades to rebuilding host dev-tools (`host:gperf`), whose `cookbook_redoxer` host-toolchain init **fails with a 404** (`redoxer: building toolchain` → `curl (22) 404` → `unable to init toolchain`) — a broken host-toolchain download, above the recipe layer. So the path is: fix/repair the local host-toolchain source build (so `host:gperf` and the netsurf chain build offline/from source) → the PIE wrapper yields a PIE `netsurf-fb` → then verify it actually runs (SDL/orbital rendering on Redox may need more). Alternative: **adopt a lighter PIE-friendly engine**. `[P2·L·Mac/QEMU]`
