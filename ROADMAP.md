...
- ⏳ `R-D10` **netsurf: web browsing over the network** — netsurf launches and renders local pages (R-D06 ✅), and click-to-launch from the bar is verified (U-105), but loading a real URL (`https://www.redox-os.org/`) shows a blank page — Redox networking (netstack/DNS/TLS, and netsurf's curl/openssl fetch path) is not functional on the aarch64/QEMU loop yet. Default homepage is `about:welcome` (local) until this works. Scope: bring up the net stack under QEMU user-net, confirm DNS + TLS, then a fetch renders. `[P2·L·Mac/QEMU]`
