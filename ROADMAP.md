...
| `R-601a` | `build-image` / `build-image-x86_64` also build the installation medium and export it as an artefact. ✅ **done 2026-09-01** (#4): the medium is built (`.gitlab-ci.yml:430`, `:534`), boot-smoked, checksummed, signed, and now published — both images compressed `zstd -3 -T0`, 345 MB of artefact against a 1 GiB cap, `201 Created` in the nightly run. Checksums stay over the RAW images: what a person verifies is the file they write to a disk. | 🔴 |
