...
| 30 | **Whether a file-backed `driver_block::Disk` daemon links and serves partitions** (`R-818`) — and whether `DiskScheme` needs an add/remove patch | in the container: build a ~60-line daemon with git dependencies `driver-block` + `daemon` on `eos-base` for `x86_64-unknown-redox`; in QEMU attach an IMG, `ls /scheme/disk.image0/`, expect `0p1…` from `partitionlib` |
