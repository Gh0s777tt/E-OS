...
- ⏳ `R-F08` **greeter/desktop VT not auto-activated on boot** — session renders fine and is reachable via `Super+F3` (verified: greeter+desktop+eos-settings); root cause = `inputd` auto-activates only the FIRST VT created (bootlog VT1 wins after the init reorg), so the greeter's VT never auto-activates. Fix: have the greeter/init call `activate_vt`. `[P1·S·mac-aarch64-qemu]`
...
- 🚧 `R-602` **First-boot OOBE wizard (retires default-creds)** — text/getty path DONE + verified for BOTH default accounts (`U-076`+`U-077`, eos-userutils `799088a`): a shared `force_first_boot_passwd` helper forces `passwd` before the shell for the passwordless `user` (`assets/screenshots/eos-oobe-firstboot.png`) AND for any account still on the shipped default `password` — order-independent, so it catches `root/password` too (`assets/screenshots/eos-oobe-root.png`). The live P0 default-creds exposure is closed on the text/getty path. Follow-up: graphical greeter (`orblogin`) login path + per-machine identity (hostname/locale/keymap/machine-id/SSH keys). — On first login of a fresh install, force set-password for the primary user, offer account creation, set hostname, timezone/locale/keyboard, regenerate machine-id + SSH host keys, and expire the default root password — eliminating the shipped passwordless `user` + `root/password` (a live, FDE-defeating exposure per eos.aarch64.toml:82/:106). `[P0·L·Mac/QEMU]` · needs `R-601`
