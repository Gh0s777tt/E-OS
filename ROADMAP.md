...
| 27 | **Whether `ahcid`'s ATAPI path reads a data disc at all** (`R-815`, `PR-022`) — the code exists, no run has ever exercised it | `qemu-system-x86_64 -M q35 -m 2G -drive file=eos-x86_64-harddrive.img,format=raw -drive if=none,id=cd0,media=cdrom,file=probe.iso -device ide-cd,bus=ide.0,drive=cd0 -serial stdio`; inside: `ls /scheme/disk.pci-*-ahci/` and `dd if=/scheme/disk.pci-*-ahci/<n> bs=2048 count=16 \| xxd \| head` — expect `CD001` at 0x8001 |
| 28 | **Whether `usbscsid` binds and reads a USB optical drive** (`R-815`) — `scsi/mod.rs:222` only changes the mode-page layout for non-DirectAccess | QEMU `-device qemu-xhci -device usb-storage,drive=usbcd,removable=on -drive if=none,id=usbcd,media=cdrom,file=probe.iso`; then `ls /scheme/disk.usb-*-scsi/` |
| 29 | **Whether `redoxfs` mounts from a plain file path on Redox** (`R-818`; `mount.rs:170` `DiskFile::open(path)` suggests yes) | in a booted E-OS: `dd if=/scheme/disk.live/0 of=/home/user/live.img bs=4096 count=4096 && redoxfs /home/user/live.img imgtest && ls /scheme/imgtest` |
...
| 32 | **[measured 2026-09-04, evening — partly] `opticaldiscs` 0.15 with `default-features = false` cooks with the cookbook CC on both arches** (`LIBCHDMAN_FORCE_SOURCE=1`, `cookbook_redoxer env`): x86_64 Finished 21.53 s, aarch64 13.86 s, both `probe-optical` binaries 400 536 B. **What that did and did not exercise:** the lock file of that probe holds `zstd-sys`, `zstd-safe`, `liblzma-sys` and `cc` — so the C in zstd and lzma compiled with the cookbook CC — but **no `chd`/`libchdman` crate at all**, because the feature set of the probe leaves CHD out; the CHD decompressors, which are the point of the question, were not built. Settle the rest with a probe that enables the CHD feature. Irrelevant to v1, which avoids the crate. *Was:* not measured | in the container: `LIBCHDMAN_FORCE_SOURCE=1 cargo build --target x86_64-unknown-redox` in `probe-E-disc-images/optical` |
| 33 | **[measured 2026-09-04, evening] None of the seven `wip/` optical recipes builds** (`CS-009`), each cooked with `make r.<recipe>` for x86_64 in the build container, 15-minute cap: `xorriso`, `libisofs`, `libburn`, `libisoburn` — `configure: error: config.sub x86_64-unknown-redox failed` (an autoconf `config.sub` that does not know the Redox triple; fails in seconds); `mkisofs-rs` — `error[E0635]: unknown feature proc_macro_span_shrink` in `proc-macro2 v1.0.46` (a 2022 lock file against the current nightly; a `cargo update -p proc-macro2` in the recipe is the obvious next probe); `libcdio` — configure passes, `make` fails (`Makefile:490: all` Error 2 after ~10 min; the compile error itself was lost to a `tail -6` in the probe script and has to be re-read from a full log); `k3b` — its dependency `libcdio-paranoia` stops at `configure: error: Required libcdio library not found`, i.e. it inherits the `libcdio` failure. Logs: `~/eos-artifacts/dowody/s15-2026-09-04/s15.log`, `s15b.log`. *Was:* never cooked | in the container: `for r in …; do ./target/release/cook wip/$r; done` — the honest outcome may be "does not build" |
