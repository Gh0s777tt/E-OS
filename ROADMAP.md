...
|---|---|---|---|---|
| **M1 — "a USB stick that installs to a real disk"** | the smallest vertical slice: named, signed medium → boots on a physical PC → installs to an internal disk → reboot **with the medium removed** → `eos login:` | `ADR-0007`, `ADR-0008`; `R-601` ✅; `V2-N03` ✅; **one physical x86_64 machine** | 🖥️🐧⚙️🔑 | 🟡 — **9 of 11 tasks closed (2026-09-01; was 3).** Open: `R-601c` (the same harness on x86_64 — it reaches the installer and stalls in the first-boot password exchange, #6) and `R-607b` (first run on metal, which needs a physical PC and cannot be closed from QEMU). |
