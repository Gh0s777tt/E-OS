...
| C2 | `PR-004` | E-OS Guard's engine as one crate — `eos-fsintegrity`, a workspace member of `eos-guard` used by `eos-control` at a pinned revision (done 2026-09-04, `eos-guard!6` + `eos-control!6`); FDE/RAID/repo status lines still open | the security product the owner asked to name; §21 proposal 3 finished | 🖥️ |
...
| **Integrity monitor** — `eos-guard` | **yes**, as an app since 2026-09-03 (`PR-002` ✅) | type A, `recipes/gui/eos-guard` pinned `a984e4c3`; `[packages.eos-guard]` in `config/x86_64/eos.toml:46` and `config/aarch64/eos.toml:44`; `config/optional-apps.toml` `[eos-guard]` ("Nie jest to antywirus.") | blake3 baseline + permission audit; one engine shared with `eos-control`'s Security tab (`PR-004`); stays a separate product (Q16) |
...
| 33 | **[measured 2026-09-04, evening] None of the seven `wip/` optical recipes builds** (`CS-009`), each cooked with `make r.<recipe>` for x86_64 in the build container, 15-minute cap: `xorriso`, `libisofs`, `libburn`, `libisoburn` — `configure: error: config.sub x86_64-unknown-redox failed` (an autoconf `config.sub` that does not know the Redox triple; fails in seconds); `mkisofs-rs` — `error[E0635]: unknown feature proc_macro_span_shrink` in `proc-macro2 v1.0.46` (a 2022 lock file against the current nightly; a `cargo update -p proc-macro2` in the recipe is the obvious next probe); `libcdio` 2.3.0 — `configure` passes but its iconv probe fails (`checking for iconv... no`; `build/config.log:2143`/`:2242` `fatal error: iconv.h: No such file or directory`, also with `-liconv`), so `config.h` has `#undef HAVE_ICONV` and the build stops at `lib/driver/utf8.c:392:3: #error "The iconv library is needed to build drivers, but it is not detected"` — a missing dependency, not a porting bug: `recipes/libs/libiconv` (1.17, `01_redox.patch`) exists in the tree and the `wip` recipe (`#TODO not compiled or tested`, `template = "configure"`) declares no `[build] dependencies`; adding it was **not** measured. Full log `~/eos-artifacts/dowody/s15-2026-09-04/libcdio-full.log` (411 lines; an incremental 4 s run on the 20:28 `build/` dir — a clean rebuild was not done, the first run's "~10 min" and this 4 s are two different runs); `k3b` — its dependency `libcdio-paranoia` stops at `configure: error: Required libcdio library not found`, i.e. it inherits the `libcdio` failure. Logs: `~/eos-artifacts/dowody/s15-2026-09-04/s15.log`, `s15b.log`. *Was:* never cooked | in the container: `for r in …; do ./target/release/cook wip/$r; done` — the honest outcome may be "does not build" |
| 34 | **Whether double-click in `cosmic-files` reaches a launcher `accept=` entry** (`R-D08`, `PR-022`) | in a booted desktop: `printf 'name=probe\nbinary=/usr/bin/echo\naccept=*.iso\n' > /usr/share/ui/apps/70_probe && touch /home/user/x.iso && launcher /home/user/x.iso`, then double-click `x.iso` in `cosmic-files` |
...
| `R-Fxx` correctness and regression | `R-F01` … `R-F34` (all 34, none omitted; `R-F30`–`R-F34` minted 2026-09-03/05) | §10 |
