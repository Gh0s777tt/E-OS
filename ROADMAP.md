...
- 🚧 `R-D06` **netsurf: build from source as PIE, get it running** — The bundled browser died the instant it was clicked (data abort, ESR 0x92000047) because the shipped binary was the upstream non-PIE `ET_EXEC` prebuilt, and aarch64-Redox only loads PIEs. **Two of the three blockers are now fixed and proven (U-103, 2026-07-19):** (1) the from-source build was blocked by a host-toolchain **404** — `host:gperf` builds via `cookbook_redoxer`, whose `toolchain()` tried to download a *host→host* relibc toolchain that redox never publishes → fixed reproducibly with `scripts/redoxer-host-stub.sh` (pre-creates the per-target stub dir so redoxer skips the misfired download; host builds use system `gcc`/`g++`); (2) a **CC-wrapper** in the recipe now forces `-fPIC` on every compile and `-pie` on the final link → `netsurf-fb` is a verified `DYN`/`pie executable`, and because our recipe differs from upstream, `--repo-binary` no longer re-downloads the prebuilt (the image always gets the source PIE). **Result:** netsurf now *loads, runs, and opens an 800×600 window* on the desktop — the load-time crash is gone. **Still open:** the window body stays black and the process then crashes during its *first content render* — a deterministic-location data abort (crash `ELR` page-offset `0x718`, `FAR` offset `0xf83`; a non-page-aligned `funmap` warning fires just before). That's a separate, deeper netsurf-on-Redox rendering bug (framebuffer/`libnsfb`, SDL-orbital surface, or freetype). Next: symbolicate via `-g` + `addr2line` (needs the PIE load base at crash time). Full write-up: `docs/design-netsurf-pie.md`. Alternative still on the table: **a lighter PIE-friendly engine**. `[P2·M·Mac/QEMU]`
