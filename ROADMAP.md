...
- ⏳ `R-F08` **orbital display output not reaching the ramfb** — after R-F07 the session starts but the greeter renders on VT3 and never reaches the QEMU framebuffer (text console stays foreground); needs vesad/fbcond/VT-arbitration diagnosis. Blocks the visible desktop. `[P0·M·mac-aarch64-qemu]`
