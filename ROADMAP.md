...
- ⏳ `R-D10` **netsurf: web browsing over the network** — netsurf launches and renders local pages (R-D06 ✅), click-to-launch verified (U-105), but a real URL shows a blank page. **Diagnosed (2026-07-19) — the whole stack below the wire is up and correct; packets just don't flow out:** virtio-netd binds the NIC and reads the MAC (link up); netstack/smolnetd runs (`ip`/`tcp`/`udp`/`icmp`/`netcfg` + the `network.pci-…_virtio_net` link scheme are all present at runtime); and `base.toml` ships a correct QEMU config — `/etc/net/ip`=10.0.2.15, `ip_router`=10.0.2.2 (SLIRP gateway), `ip_subnet`=/24, `dns`=9.9.9.9. The host sandbox has full egress (verified: HTTPS **and** external UDP:53 to 9.9.9.9/8.8.8.8/1.1.1.1 all succeed), so QEMU user-net can reach the internet. **Yet inside Redox `curl` DNS resolution times out** (`curl: (28) Resolving timed out after 20074 ms`) **and a raw-IP TCP connect (`http://9.9.9.9/`) also never returns** — so it is *not* just DNS: the actual packet TX/RX to the outside fails. That points at Redox's data path (smolnetd ↔ virtio-netd TX/RX, or ARP to the gateway), not config, driver-init, TLS, or the sandbox. **Next step:** boot with a QEMU `-object filter-dump,netdev=n0,file=…pcap`, trigger a query, and check whether frames leave the guest at all — that splits "Redox not transmitting" from "SLIRP not answering" — then trace virtio-netd's TX/RX rings or smolnetd's route/ARP on aarch64. `[P2·L·Mac/QEMU]`
