...
| `R-F34` | **`dd` in the image refuses to run before it touches a file — and `status=none` is the whole workaround** (minted 2026-09-05, cause found the same day). `/usr/bin/dd` (`dd (uutils coreutils) 0.7.0`, recipe `core/uutils` pinned `1f7c81f5`) prints `dd: write error` and exits 1 for `of=FILE`, `> FILE` and `\| cat > FILE` alike, with inputs `/dev/zero`, the NVMe disk, an ATAPI disc and a regular file — 12 invocations across four guest sessions, never a byte written. **Cause:** `dd.rs:1516-1519` at the pinned revision is a *startup guard*, not an I/O path — `if uucore::signals::stderr_was_closed() && settings.status != Some(StatusLevel::None) { return Err(USimpleError::new(1, "write error")) }` — placed before the input and output are even opened, which is why every variant fails identically and no output file is created. `stderr_was_closed()` reads a static that an `.init_array` constructor fills in before `main`: `signals.rs:490-493` stores `fcntl(STDERR_FILENO, F_GETFD) == -1`. So on this image that `fcntl` answers −1; by elimination the constructor did run (otherwise the static would stay `false` and `dd` would work). **Measured 2026-09-05** on a disk installed from the `!133` image (`dowody/rf34-dd-2026-09-05/dd-variants-serial.log`, one boot, ten invocations): `bs=512 count=1` → `dd: write error`, `$?` = 1, no file; same with `conv=notrunc`, with `bs=1 count=10`, with a regular file as input, with no operands beyond `if`/`of`, with `> file` (the shell leaves a 0-byte file) and even writing to the console (`bs=8 count=1`, no `of=`); **`status=none` writes: `/tmp/d2.bin` is 512 B** — the one variant the guard lets through. Control in the same boot: `head -c 512 /dev/zero > /tmp/h1.bin` → 512 B. **Consequence:** every §15 recipe and every Redox document that says `dd of=` works on this image **if `status=none` is added**; nothing about the write path is broken. **Still open [UNVERIFIED]:** why `fcntl(2, F_GETFD)` fails — relibc forwards every unhandled `cmd` to `redox_rt::sys::fcntl` (`platform/redox/mod.rs:334-445`, `F_GETFD` has no arm of its own), so the refusal is one layer below relibc's C entry point; the next probe is a ten-line Rust program that calls `fcntl(2, F_GETFD)` and prints `errno` (the row-30 kit builds and delivers such a probe in one boot). Evidence: `dowody/rf34-dd-2026-09-05/` (serial log, both upstream sources at the pinned revision) and the original sightings in `dowody/s15-qemu-2026-09-05/{kit3,kit4,rows-24-25-29-run2,row-27-ide-cd-serial-run2,verify-29}-serial.log` | 🔴 |
