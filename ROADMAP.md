...
- ⏳ `R-F11` **13 recipes reach the shipped image without a pin or a hash** — of the **74** packages actually in `repo/x86_64-unknown-redox`, **10** have a `git =` source with no `rev`/`branch`/`tag` (`ca-certificates`, `cosmic-edit`, `cosmic-files`, `cosmic-term`, `bottom`, `findutils`, `hicolor-icon-theme`, `pop-icon-theme`, `ripgrep`, `sdl1`) and **3** fetch a tarball with no `blake3` (`file`, `nano`, `openssh`). `ca-certificates` is the worst of them: an unpinned TLS trust root. Tree-wide the ratio is 1527/2051 and 770/1032, but those are inherited third-party ports and are explicitly out of scope (CLAUDE.md §3) — **this item is only the 13 that ship**. Also fix the enclosing defect: a missing hash is currently a `WARNING`, so add a `bail` behind `EOS_STRICT_FETCH=1` and set it in CI. `[P0·M·any]`
