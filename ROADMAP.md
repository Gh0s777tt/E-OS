...
| `R-F31` | **`nvmed` partition entries (`1pN`) return a wrong sector on seek-then-read** (minted 2026-09-05 from §15 row 24). On the x86_64 installer image booted in QEMU 11.0.2 the boot disk lists `1 1p0 1p1 1p2` and a sequential read of the whole ESP through `1p1` (`head -c 1048576`) is byte-correct — `fatfs` lists `EFI/BOOT/BOOTX64.EFI` from it — but the same `fatfs` probe pointed directly at `1p1` reads a correct boot sector and then a mangled root-directory sector at partition offset 6656 (16 identical slots made of the `EFI` entry's LFN bytes, size field `41 45 00 46`), so `open_dir(EFI/BOOT)` fails; reproduced in two guest sessions (`dowody/s15-qemu-2026-09-05/rows-24-25-29-run3-serial.log`, `verify-24-serial.log`). `od -N 64 -j N` on `1p1` and on `1` fails with `EINVAL`, so the positional read could not be compared with the whole-disk device and the faulty layer (`nvmed`, `partitionlib`/`driver-block` offset handling, relibc `lseek`+`read`) is not isolated. Blocks any feature that reads the ESP through `1pN` at an offset (`R-707` boot flag/counter, ADR-0009 D7); sequential reads are the known-good path until fixed. Next probe: a 20-line reader doing `pread` vs `lseek`+`read` at offsets 0, 512, 6656 on `1` (+1 MiB) and `1p1`, both aligned and unaligned | 🔴 |
| `R-F32` | **`ahcid`/`driver-block` pass a read that crosses the end of an ATAPI medium through unclamped, so the last blocks of a disc are unreadable by ordinary tools** (minted 2026-09-05 from §15 row 27). With `probe.iso` (450 × 2048 B) on the ICH9 AHCI port the disc binds (`disk.pci-0000-00-1f.2_ahci/0`, `stat` size 921600, IO block 2048) and READ(10) data is bit-exact for blocks 0–447 (sha256 of the first 917504 B equals the host's), but every whole-device read (`cp`, `cat`, `sha256sum`) stops at 917504 B with `[@ahcid::ahci::hba:451 ERROR] IS 40000000 IE 17 CMD 3000006 TFD 5041` (sense key 5, ILLEGAL REQUEST) and `I/O error`: the reader's 8 KiB request at block 448 runs past block 449 and `driver-block/src/lib.rs` `read()` forwards it as is (only `buf.len() % block_size != 0 → EINVAL` is checked); a request ending exactly at the medium end (`tail -c 4096`) succeeds. Source-level suspect, not measured: `disk_atapi.rs` `read()` advances `sector += blk_len` for requests ≥ 64 blocks. Fix shape: clamp the request to the device size in `driver-block` and return a short read. Evidence: `dowody/s15-qemu-2026-09-05/row-27-ide-cd-serial-run2.log`, `verify-27-serial.log` | 🔴 |
| `R-F33` | **`usbscsid` binds a USB optical drive and answers INQUIRY/READ CAPACITY, but no block read ever returns and the reader cannot be interrupted** (minted 2026-09-05 from §15 row 28). QEMU `usb-storage,removable=on` with `probe.iso` on `qemu-xhci`: `xhcid` loads the subdriver, `usbscsid` reports `Inquiry version: 5` and `read_capacity10 … max_lba 449, block_len 2048`, the scheme `disk.usb-usb.pci-0000-00-02.0_xhci+2-scsi/0` appears with size 919552 — one block short, because `ReadCapacity10ParamData::block_count()` returns the last LBA instead of last LBA + 1 (`usbscsid/src/scsi/cmds.rs:452-453`) — and then `od -j 32768 -N 64`, `head -c 2048` and the `fatfs` probe all block indefinitely (813 s in one session, still `Running` after 8 min in another); `^C` does not recover the shell; no `usbscsid: READ IO ERROR` line ever appears, only one `xhcid: Lost event TRB type 32, completion code: 1`. Whether READ(16) was ever issued was not separated by a control. Two off-by-one/blocking defects, not one. Evidence: `dowody/s15-qemu-2026-09-05/row-28-usb-cd-serial.log`, `verify-28-serial.log` | 🔴 |
| `R-F34` | **`dd` in the image cannot write anything** (minted 2026-09-05; hit by every §15 recipe that says `dd of=`). `/usr/bin/dd` (`dd (uutils coreutils) 0.7.0`) prints `dd: write error` and exits 1 for `of=FILE`, `> FILE` and `| cat > FILE` alike, with inputs `/dev/zero`, the NVMe disk, an ATAPI disc and a regular file — 12 invocations across four guest sessions, never a byte written (0-byte or no output file); `head -c` and plain shell redirection to the same targets work. Which syscall fails was not measured (no strace; relibc printed no `TODO` line). Consequences: the row-29 recipe (`dd if=/scheme/disk.live/0 …`), the row-24 ESP extraction and any user following Redox documentation that uses `dd`. Next probe: run `dd` variants (`conv=notrunc`, `oflag=`, `status=none`, `bs=512`) and read uutils `dd.rs` for the branch that prints the bare `write error` (an `Err` from `write`/`flush`/`fsync`/`ftruncate`?). Evidence: `dowody/s15-qemu-2026-09-05/{kit3,kit4,rows-24-25-29-run2,row-27-ide-cd-serial-run2,verify-29}-serial.log` | 🔴 |
...
| `R-F36` | **The kernel panics on the shutdown path — `attempt to subtract with overflow` in `userspace_acpi_shutdown()`** (minted 2026-09-05). In **2 of 5** guest shutdowns measured today (all on disks installed from the `!133`-branch image, QEMU 11.0.2), root's `shutdown` produces `KERNEL PANIC: panicked at src/arch/x86_shared/stop.rs:88:12: attempt to subtract with overflow` immediately after `kernel::arch::x86_shared::stop:INFO -- Waiting one second for ACPI driver to run the shutdown sequence.`; the backtrace runs `syscall_instruction → sys_write → SysScheme::kwriteoff → FILES::{closure#1} → panic_handler_inner`, names the faulting process (`NAME: /usr/bin/shutdown`, `SYSCALL: write(5, …, 8)`) and ends in `HALT` on CPU #1. Line 88 is `if current - initial > time::NANOS_PER_SEC`, where both values come from `time::monotonic(token)` inside the one-second wait loop — an unchecked subtraction that traps when `current < initial`. It traps **because of an E-OS hardening choice**: the kernel's `[profile.release]` sets `overflow-checks = true` (`recipes/core/kernel/source/Cargo.toml:117-123`, "turn silent integer overflow … into a controlled abort"), so where upstream wraps silently, E-OS aborts. Why the monotonic clock appears to go backwards is **[UNVERIFIED]**. The machine still powered off in both runs — `acpid` had already written S5 itself (`[@acpid::acpi:636 WARN] Shutdown with ACPI outw(0x604, 0x2000)`, QEMU exited 2.7 s after `shutdown`) — but the panic aborts `kstop()` **before** its own fallbacks ("Timeout reached, thus falling back to other shutdown methods"), so on a machine where the ACPI driver is absent, slower or crashed, nothing else runs. Fix shape (one line in our fork `eos-kernel` `68a51035`, branch `eos-july`): `current.saturating_sub(initial)` or `current > initial + NANOS_PER_SEC`; a boot smoke plus five shutdowns would be the gate. Evidence: `dowody/rf35-id-eperm-2026-09-05/{run1,run2}/serial.log` (panicked) vs `dowody/s15-row34-2026-09-05/{run1,run2}/serial.log` and `dowody/pr004-onimage-2026-09-05/new-disk-serial-selftest/serial.log` (same command, no panic) | 🔴 |
...
| 30 | **[measured 2026-09-05 — settled] A file-backed `driver_block::Disk` daemon written OUTSIDE `eos-base` links, runs and serves the file's partitions, and `DiskScheme` needs no patch for that — but it has no add/remove API at all** (`R-818`). The probe `imgd` (127 lines, `dowody/s15-row30-2026-09-05/crate/`) depends on `driver-block` **by package name from the eos-base git repository** (a workspace member, the `eos-credpolicy` pattern) and cross-builds with the cookbook toolchain in 11 s: `cookbook_redoxer env cargo build --release --target x86_64-unknown-redox` → 591 736 B ELF x86-64, static; the graph pulls `daemon`, `executor`, `scheme-utils`, `partitionlib`, `redox-scheme` and `gpt` from the same checkout (`816546df`). **Trap:** `daemon::Daemon::new` refuses a manual start — `Daemons can't be started manually. Add a service config to make init start this daemon instead.` (`daemon/src/lib.rs:16` panics when `$INIT_NOTIFY` is unset; the E-OS patch turns an *invalid* fd into `None`, so `INIT_NOTIFY=99` also gets past it) — but `DiskScheme::new` takes `Option<Daemon>`, so a shell-started probe passes `None` and simply never signals readiness. In the guest (installed disk from `e-os!133`, one boot, files fetched over slirp): `imgd /tmp/probe.img &` → `imgd: serving /tmp/probe.img (1048576 B, 512 B blocks) as disk.image0`, and the daemon's own `read` is called at once with `block=1` then `block=2` — `partitionlib` reading the GPT header and entries. `ls /scheme/disk.image0` → **`0  0p0  0p1`**, `ls -la` sizes `1048576 / 114688 / 114688` = the whole file and the two GPT partitions of the probe image, so **the row's expectation `0p1…` is wrong in one detail: partitions are numbered from zero** (`0p0` is the first), the same shape the real NVMe disk shows (`1 1p0 1p1 1p2`). Content proves the offsets are applied: `head -c 512 /scheme/disk.image0/0p0 > /tmp/b.bin` → `imgd: read block=64 len=512` (LBA 64 = partition 1) and `b.bin` starts `ROW30-P1-DATA-…`; `head -c 1024 …/0p1` → `read block=288 len=1024` (LBA 288 = partition 2), `ROW30-P2-DATA-…`. **I/O must be a whole multiple of the block size:** `head -c 42` on `0`, `0p0` or `0p1` fails `Invalid argument (os error 22)` with **no** `imgd: read` line at all — the scheme layer rejects it before the disk sees it — and `echo … > 0p1` (15 B) fails the same way; an aligned write was **not** measured. **Add/remove:** `impl<T: Disk> DiskScheme<T>` exposes only `new`, `event_handle` and `tick` (plus `on_close`), and the disk map is moved in at construction — there is no public way to add or remove a disk afterwards, so hot-plug does need a patch; nothing here proves what that patch should look like. Logs and the built binary: `dowody/s15-row30-2026-09-05/` (`runs/run{8,10,11}-serial.log`, `imgd-x86_64-unknown-redox`, `probe.img`) | in the container: build a ~60-line daemon with git dependencies `driver-block` + `daemon` on `eos-base` for `x86_64-unknown-redox`; in QEMU attach an IMG, `ls /scheme/disk.image0/`, expect `0p1…` from `partitionlib` |
...
| `R-Fxx` correctness and regression | `R-F01` … `R-F36` (all 36, none omitted; `R-F30`–`R-F36` minted 2026-09-03/05) | §10 |
