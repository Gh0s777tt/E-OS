...
The fifteen questions are kept with their answers, because each answer is now a rule and the row it
...
| `RH-018` | **Seven product repositories have CI files and no CI.** Measured 2026-09-03 on the last `main` pipeline of each: `eos-sheets`, `eos-slides`, `eos-drive`, `eos-store` each have **7 jobs killed by `ci_quota_exceeded`** (fmt, clippy, test, coverage, cargo-deny, osv-scanner, secret-scan) and their one self-hosted job, `host-gui`, comes back **`skipped`** because it sits in a later stage than the jobs that just died. `eos-guard`, `eos-control` and `eos-ui` show 2 quota-killed and **nothing that ran at all**. So every product carries a coverage floor, a `clippy -D warnings` and a test suite that **have never executed on a merge request**. The `package-*` jobs added by `PR-008` are the only gates in these repositories that actually run, and only because they were given `tags: [eos-heavy]` **and** `needs: []` | 7 repositories × ~7 gates, all decorative; the coverage floors in particular (`--fail-under-lines 55` in `eos-sheets`) have never been evaluated once | one `checks-heavy` job per repository on `eos-heavy` with `needs: []` — fmt, `clippy -D warnings`, `cargo test`, `cargo llvm-cov` at the repository's floor — plus `needs: []` on the existing `host-gui`/`build-host` so they stop being skipped, plus `RH-017`'s `workflow:` guard so each push costs one pipeline instead of two | `RH-017` | M |
