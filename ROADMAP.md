...
- ⏳ `R-D11` **Privileged power actions from the GUI (reboot / shutdown)** — eos-control's Zasilanie tab renders and its two-step confirm works (U-108), but the action doesn't take the machine down: `sys:kstop` is root-only, eos-control runs as the desktop user (direct write → EPERM), and `sudo` from a GUI fails because it reads its password from a TTY the GUI has no access to. Proven the mechanism works: **root `shutdown -r` restarts the VM** (serial shows UEFI re-running; note a separate EDK2 warm-reboot firmware flake under QEMU-aarch64). **De-risked (2026-07-23):** the "just write an empty password to `/scheme/sudo`" shortcut is out — the desktop user's password is **not** empty (first-boot sets it; a shell login as `user` with an empty password returns `Login incorrect`), so any elevation needs the *real* password, which eos-control can't know. So a proper fix is a genuine feature: a **GUI password-prompt dialog** feeding the in-`/scheme/sudo` elevation dance (open the scheme, write the entered password, elevate the process fd + `setns` the way `sudo` does internally), then write `sys:kstop`. Alternative: a privileged helper daemon started at boot that the GUI signals (no per-action password). Neither is a quick fix; until one lands the buttons report honestly and point to the terminal. `[P2·M·Mac/QEMU]`
