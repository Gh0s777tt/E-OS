...
|---|---|---|
| Redox (E-OS) | **measured 2026-09-04**, `aarch64-unknown-redox`: `aho-corasick` + `memchr` + `blake3` + `walkdir` **7.73 s**, `regex` **8.01 s** | **no** — needs a file-event bus the kernel does not have (§7.3) |
...
**What was not measured, stated so nobody generalises it:** only `aarch64-unknown-redox` exists in
the build container's `redox` toolchain, so `x86_64-unknown-redox` is **unproven** for the scanner
stack. The image builds for x86_64 through the cookbook's own per-arch flow, which is a different
path from this probe — and §5.9's own rule is that measuring one command does not license a claim
