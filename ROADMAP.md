...
- ⏳ `R-F16` **only ONE legacy INTx line works at a time — a second PCI device silently stops the boot** — Found by the `R-601` harness (`U-146`). aarch64 has no MSI/MSI-X, so every PCI driver takes a legacy INTx line; a driver whose line **differs from the one already in service never receives an interrupt**, so it never signals readiness. `pcid-spawner` blocks per device in `Daemon::spawn` and runs as a `oneshot` unit, so `40_drivers.target` never completes → `50_rootfs.service` never runs → `init` never reaches `switchroot`. The boot **stops silently in initfs, before the root filesystem is mounted** — no panic, no error. **Nine boots, and the model predicts every one:** the source disk at slot `0x4` is line 0; a second disk at `0x5`/`0x6`/`0x7`/`0x9` (lines 1/2/3) stalls, at `0x8`/`0xC` (line 0, shared) boots. **Two controls make it a diagnosis, not a guess:** a *lone* disk moved to line 1 boots — so no individual line is dead, it is two lines at once that fails; and a `virtio-blk` second device stalls identically — so it is not an `nvmed` bug but the shared INTx path under both drivers. The `raid1d`-holds-the-target-disk theory that `R-601` carried for months was **tested and disproved** (a *formatted* second disk stalls exactly like a blank one). Impact: any aarch64 machine with two storage controllers cannot boot. x86_64 is *expected* to be unaffected because MSI/MSI-X avoids this path — **unverified**, no x86_64 image has been built on the current host. Reproduce: `scripts/repro-intx-lines.sh <image>`. Fix lives in the kernel/pcid INTx path (`eos-kernel`/`eos-base`), not in the drivers. **Open, and not assumed either way:** README advertises a `raid1d` RAID-1 mirror *over two disks* and `R-501`'s plan describes verifying it with two NVMe disks — on aarch64 that is precisely what this defect breaks unless both members share an INTx line. The `U-061`/`U-065` entries cited for that verification resolve neither in `main` nor on the archived branch, so re-running the raid1d sequence (or locating the evidence) is the way to settle whether the README claim still holds. `[P0·M·Mac/QEMU]`
