...
| `R-F29` | **`scripts/eos-build.sh:72` runs the build through `tail -3` inside `bash -lc` without `set -o pipefail`.** *Newly minted in this merge; neither predecessor carried it as an item.* Line **59** sets `set -o pipefail` for the host-tools build; line **72**, which runs `make CI=1 ARCH=$ARCH CONFIG_NAME=eos all build/$ARCH/eos/$MEDIUM_NAME 2>&1 \| tail -3`, does not — so the build's exit status is `tail`'s. This is `CLAUDE.md` trap **P-3** live in the build script, and it is the same mechanism that let `cargo: command not found` scroll past under `set -e` (`U-224`). The bootloader recipe's own comment names the output half of this defect — *"`scripts/eos-build.sh` pipes make through `tail -3`, so the warning never reached the operator"* — while the exit-status half is still open. **Stated precisely, because the script is not defenceless:** line 72 has no `\|\|` guard, so a failed `make` returns `tail`'s zero; what catches the worst case is a *downstream* check comparing the mtimes of `harddrive.img` and the medium before and after, which refuses to export unchanged images when a Secure Boot key is present. That guard fires only when **both** images are untouched, so a `make` that fails **after** writing one of them still passes. Fix: add `set -o pipefail` on the container side of line 72, as line 59 already does. **Done, and the row above was stale when it was written this way — verified 2026-09-07.** The make invocation is line **92**, not 72, it already opens with `set -o pipefail; `, and the whole `podman run` is the condition of an `if ! …; then echo "!! make FAILED…"; exit 1; fi` block at `:89-99`; `:77-88` carry the reasoning and the 2026-09-02 measurement behind it. What had never been run is the negative control this row asks for. Run now: the `podman run` from `:89-92` verbatim — same image, mounts and environment — with only the make **target** replaced by one that cannot exist, then again with only the string `set -o pipefail; ` removed. Same failing make, same three lines of output, **exit status 2 with the clause and 0 without it**. One clause decides whether the caller is told. **Boundary, because this is not the whole claim:** it measures the command, not the script — `MEDIUM_NAME` is computed at `:38` by asking make, so no environment variable can aim the real script at a failing target, and that the `if !` turns a non-zero status into `exit 1` is read four lines below the command rather than observed. No CHANGELOG entry: no code changed, this is a verification of a fix that was already in the tree. Evidence: `dowody/eos-build-pipefail-2026-09-07/` `[P1·XS·🖥️]` | ✅ |
