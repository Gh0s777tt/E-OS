...
- 🚧 `R-602` **First-boot OOBE wizard (retires default-creds)** — text/getty path DONE + verified (`U-076`, eos-userutils `b12240d`): passwordless `user` login now forces `passwd` before the shell (`assets/screenshots/eos-oobe-firstboot.png`). Follow-up: graphical greeter (`orblogin`) blank-pw path + root's weak default + hostname/locale/keymap. — On first login of a fresh install, force set-password for the primary user, offer account creation, set hostname, timezone/locale/keyboard, regenerate machine-id + SSH host keys, and expire the default root password — eliminating the shipped passwordless `user` + `root/password` (a live, FDE-defeating exposure per eos.aarch64.toml:82/:106). `[P0·L·Mac/QEMU]` · needs `R-601`
