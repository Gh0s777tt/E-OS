...
| `R-601c` | The same harness runs on **x86_64**. ✅ **done 2026-09-02** (#6): a clean x86_64 build runs `scripts/ci-install-smoke.sh` through to *"PASS — installed to a second disk and booted it to a login prompt"*, exit code 0, including stage 2 (the installed disk boots on its own). The earlier stall was **not** a stray newline left by `login`: instrumentation that drained stdin immediately before `passwd` found the queue **empty**. The responsible change is `266c4f4` in `eos-userutils` — the `getty` terminal-size probe swallowed bytes the user had typed (!49). Negative control: revert that one commit in the build tree only, rebuild, rerun → the same harness reports *"saw a rejected login"* and FAILs `[P0·M·🖥️]` | **WORKS TODAY** | ✅ |
...
| 6 | `R-601c` ✅ | the same harness on x86_64 | ✅ met 2026-09-02 (#6) — a clean x86_64 build runs the harness to *"PASS — installed to a second disk and booted it to a login prompt"*, exit 0, stage 2 included. Negative control: revert `266c4f4` (the `getty` fix) in the build tree only → *"saw a rejected login"*, FAIL | `[P0·M·🖥️]` |
