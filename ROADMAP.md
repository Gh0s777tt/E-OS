...
| C2 | `PR-004` | E-OS Guard's engine as one crate — `eos-fsintegrity`, a workspace member of `eos-guard` used by `eos-control` at a pinned revision (done 2026-09-05, `eos-guard!6` + `eos-control!6`); FDE/RAID/repo status lines still open | the security product the owner asked to name; §21 proposal 3 finished | 🖥️ |
...
| 31 | **Whether the pure-Rust image crates link on both Redox arches** (`R-819`, `PR-022`) — only `cargo check` for `x86_64-unknown-redox` was measured on the host; `aarch64-unknown-redox` has no host `rust-std` | in the container: `cp -r probe-E-disc-images/pure` and `cargo build --release --target x86_64-unknown-redox && cargo build --release --target aarch64-unknown-redox` |
| 32 | **Whether `opticaldiscs`' C decompressors cook with the cookbook CC** once the prebuilt download is disabled (`recipes/archives` has `xz` and `zstd`) — irrelevant to v1, which avoids the crate | in the container: `LIBCHDMAN_FORCE_SOURCE=1 cargo build --target x86_64-unknown-redox` in `probe-E-disc-images/optical` |
| 33 | **Whether any `wip/` optical recipe builds** (`CS-009`): `tools/xorriso`, `tools/mkisofs-rs`, `libs/other/libcdio`, `libs/other/libisofs`, `libs/other/libburn`, `libs/other/libisoburn`, `kde/k3b` | in the container: `for r in …; do ./target/release/cook wip/$r; done` — the honest outcome may be "does not build" |
