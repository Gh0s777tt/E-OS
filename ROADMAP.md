...
- ⏳ `R-F05` **Fix numbering + copy-paste doc drift** — *(Partial, 2026-07-23: the **duplicate U-039 is resolved** — the netsurf crash is now recorded under `U-103`/`U-104` (R-D06, and marked ✅ in `docs/known-issues.md`), so `U-039` unambiguously means the upstream-patch refresh.)* Still open: close the missing **U-038** gap per CLAUDE.md 'bez luk' and define the going-forward `R-NNN`↔`U-NNN` mapping; fix the copy-pasted aarch64-only comment in eos.x86_64.toml, the lived null-namespace expect() message, and the no-op Transaction::remove io::sink hash. `[P2·S·any]`
...
- 🚧 `R-902` **Graphical Network Settings pane** — **Static apply + live read done** (`U-112`, eos-control `9e95c32`): the eos-control **"Sieć"** tab reads the **live `netcfg:` scheme** (interface, IP/netmask, default-route gateway, DNS, MAC, link status; `/etc/net/*` fallback) and **applies a static IPv4 config live** — IP/prefix/gateway/DNS, written to the root-only `netcfg:` scheme through a new privileged **`eos-netcfg`** shim (password-gated, GUI never runs as root, like `eos-power`). Contract recon'd from the smolnetd source; host build + `--selftest` green; live apply + render **gated to CI/boot** (not HW-blocked — wired net works on QEMU, `R-D10`). See `docs/design-eos-control-network.md`. **Remaining:** a persistent **DHCP/static toggle** (`dhcpd` lifecycle + `/etc/net/*`, ties into `R-906`) and the same pane in the **installer** front-ends (`R-603`). `[P1·M·Mac/QEMU]` · needs `R-D01`
