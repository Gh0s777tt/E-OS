...
|---|---|---|---|---|
| **M1 — "a USB stick that installs to a real disk"** | the smallest vertical slice: named, signed medium → boots on a physical PC → installs to an internal disk → reboot **with the medium removed** → `eos login:` | `ADR-0007`, `ADR-0008`; `R-601` ✅; `V2-N03` ✅; **one physical x86_64 machine** | 🖥️🐧⚙️🔑 | 🟡 — 3 of 11 tasks closed |
...
**M1 status, measured 2026-08-31:** 11 tasks tracked as GitLab issues, **3 closed** (#1 `R-611a`, #7 `R-607a`, #8 `R-612a`), 8 open. Tasks 1 (`R-611a`), 7 (`R-607a`) and 8
...
> `repos.toml:116` = `74726c889bdf61ff683558227fe95178061a88e8` and `repos.toml:176` =
...
|---|---|---|
| Bootloader source public and pinned | ✅ | `recipes/core/bootloader/recipe.toml:4-6` → `eos-bootloader` rev `b249982f`; toolchain pinned `nightly-2026-05-24` |
...
| `V2-MS04` | **Secure Boot gate in CI** — wire `eos-secureboot-proof.sh` into `.gitlab-ci.yml` | the proof stops depending on one laptop. **Downstream of `R-009`**: with both CI platforms dead this changes nothing until capacity returns | 🐧 | 🔴 |
...
| `R-601a` | `build-image` / `build-image-x86_64` also build the installation medium and export it as an artefact. Measured today: `grep -c "redox-live\|installer.img" .gitlab-ci.yml` → **0**. Negative control: remove the step → job red, no artefact `[P1·S·🐧]` | **BUILDABLE** | 🔴 |
| `R-601b` | `scripts/ci-install-smoke.sh` starts **from the medium**, not from `harddrive.img`, and is wired into scheduled CI. Measured today: `grep -c "install-smoke" .gitlab-ci.yml` → **0**. Negative control: run the harness against an image with no installer → FAIL, not "PASS out of silence" `[P1·M·🐧]` | **BUILDABLE** | 🔴 |
| `R-601c` | The same harness runs on **x86_64**. Measured today, and the citation is exact: `scripts/ci-install-smoke.sh:32` → `[ "$ARCH" = "aarch64" ] \|\| { echo "install-smoke: only aarch64 is wired up"; exit 2; }`. That `exit 2` **is** the negative control before the fix `[P0·M·🖥️]` | **BUILDABLE** | 🔴 |
...
| `R-604a` | Disk picker shows **device path, size, interface type and removability**; erase is confirmed by **retyping the device path**, not by picking a number. Negative control: two disks in QEMU, type the wrong name → refusal, zero writes. Today the harness expects the literal `Select a drive from 1 to`, and **the number in that menu changes between runs** if PCI enumeration order changes `[P0·M·🖥️]` | **BUILDABLE** | 🔴 |
...
| `R-608` | **Correct `docs/getting-started/install.md` so it matches the shipped installer.** §2 claims the installer walks through creating users and passwords and choosing the package set; the built binary does neither. Scope corrected 2026-08-15 (`U-126`): this ticket also used to accuse §2 of inventing an interactive encryption walkthrough — that part was **wrong**, the `redoxfs password` prompt is real, so the encryption bullet stays. The `needs R-603` dependency was likewise dropped: this is a text-only fix to already-shipped behaviour and was pointlessly blocked behind a hardware harness. M1 additionally requires the artefact **name and write procedure** to be true after `R-611a` — `grep -c 'redox-live.iso' docs/getting-started/install.md` → 0 `[P1·S·🖥️]` | **BUILDABLE** | 🔴 |
...
| 6 | `R-601c` | the same harness on x86_64 | `ci-install-smoke.sh:32` `exit 2` for any non-aarch64 arch — that line **is** the negative control | `[P0·M·🖥️]` |
...
| 9 | `R-604a` | disk picker identifies the disk; confirm by retyping the path | two disks in QEMU, wrong name typed → refusal, zero writes | `[P0·M·🖥️]` |
| 10 | `R-608` *(part)* | `install.md` tells the truth about the artefact name and write procedure | `grep -c 'redox-live.iso' docs/getting-started/install.md` → 0 | `[P1·XS·🖥️]` |
...
| `R-916` | **I2C bus subsystem plus I2C-HID.** Redox has **no I2C bus driver at all** (`HARDWARE.md:40`); this single gap blocks the entire sensor category, most laptop precision touchpads, and Type-C PD controllers. Foundational bus driver first, then I2C-HID. Same work as `V2-N01`; a precondition of `R-932` and `R-935` `[P3·L·⚙️]` | T3 blocker | 🔴 |
...
| **Restore CI capacity — every gate has been dark since 2026-08-28** | **High** | `C-7` | **`R-009`** (§11) |
...
- **`ci-integrity.sh` has checks 0…12** — verified this session: banner `0.` (the instrument probe)
  ...
  check 12** — see Annex B. *`CLAUDE.md` §0 and §13 still speak of **eight** checks; the
  documentation is four behind the gate, which is a defect against `CLAUDE.md` §5.8, to be fixed
...
- **Every gate in this project is enforced only on the developer's machine** (`R-009`). Both CI
  platforms are dead: GitLab aborts in ~0 s on `ci_quota_exceeded`, GitHub Actions produces no runs
  at all. Every "in CI" clause in this document is therefore a statement about a **file**, not about
  an executed check, until `R-009` closes.
...
register did not contain this item while `EP-2` and `M4` depended on it, and `CLAUDE.md:575`,
