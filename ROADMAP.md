...
- 🚧 `R-D05` **Launcher search + local-time clock** — Clock DONE (`U-098`, eos-orbutils `94dcc91`): the bar reads local `YYYY-MM-DD HH:MM UTC±H` from `/etc/tz-offset` (default UTC; ships 7200/UTC+2), render-verified `12:58 UTC+2` at host-UTC 10:58. Remaining: type-to-search in the Start menu (filter apps as you type). Original: add type-to-search filtering to the Start menu and fix the clock (was UTC HH:MM only) to show local date + timezone. `[P2·S·Mac/QEMU]`
