...
- ⏳ `R-702` **Pin the repo pubkey; kill TOFU** — No E-OS pubkey is baked into any config (grep=0 hits) so remote update keys are fetched TOFU from the same host that serves packages, defeating the per-package signature on first contact; pin the pubkey into the image, enforce https:// in add_remote, verify the pubkey-cache provenance, and stop fetching the key over the package channel. **This is the whole bottleneck of the trust chain and it depends on nothing**: `pkg-lib` already verifies the manifest, the publisher already signs — the only missing artifact is `keys/eos-repo-sign.pub.toml`, which `keys/README.md` documents how to generate. `[P0·M·any]`
