...
| `R-601a` | `build-image` / `build-image-x86_64` also build the installation medium and export it as an artefact. Updated 2026-09-01: the medium **is** built (`.gitlab-ci.yml:430`, `:534` — `make -s print-installer-medium`). The old probe `grep -c "redox-live"` → **0** is still true and now meaningless: the medium is called `eos-<version>-<arch>-installer.img`. What remains open is **only the export as a downloadable artefact** — two 1.4 GiB images exceed the 1 GiB per-job cap, so the job publishes `sha256sums-<arch>.txt` and the SBOM instead. Negative control: remove the step → job red, no artefact `[P1·S·🐧]` | **BUILDABLE** | 🔴 |
| `R-601b` | `scripts/ci-install-smoke.sh` starts **from the medium**, not from `harddrive.img`, and is wired into scheduled CI. ✅ **done 2026-09-01** (#5): `grep -c "install-smoke" .gitlab-ci.yml` → **10**; the harness starts from `$MEDIUM` in the nightly `build-image` (`:459`, `:569`) and passed end to end on aarch64 — `install-smoke: PASS — installed to a second disk and booted it to a login prompt`. Negative control: run the harness against an image with no installer → FAIL, not "PASS out of silence" `[P1·M·🐧]` | **BUILDABLE** | 🔴 |
| `R-601c` | The same harness runs on **x86_64**. Updated 2026-09-01: the `only aarch64 is wired up` barrier is **gone from the script** — there is an explicit `x86_64)` branch (`ci-install-smoke.sh:46`), and `exit 2` is reserved for an **unknown** arch. What remains open is the **run**: x86_64 reaches the drive menu and times out there, because serial input drops characters. That `exit 2` **is** the negative control before the fix `[P0·M·🖥️]` | **BUILDABLE** | 🔴 |
...
| 6 | `R-601c` | the same harness on x86_64 | the harness handles both arches (`ci-install-smoke.sh:46`); what is open is the **run** — x86_64 times out at the drive menu because serial input drops characters (#6) | `[P0·M·🖥️]` |
