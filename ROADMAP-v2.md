...
|---|---|---|---|---|
| **V2-STD01** 🖥️ | **POSIX (ISO/IEC 9945)** — zmierzyć `relibc` zestawem `os-test` | 🟡 **ZMIERZONE** (`U-221`), dwie liczby **celowo nie sklejone**: **deklaracje 3112/4147 = 75 %**, **wykonanie 704/830 = 84 %**. Pierwsza to sufit (Sortix: `relibc` zaślepia interfejsy — linkują się, nie działają), druga dotyczy tylko tego, co dało się uruchomić. **Znalezione 3 wieszacze spoza listy upstreamu** — `pthread_kill`, `timer_settime`, `pause`, wszystkie w sygnałach i timerach. Zostaje: końcówka `unistd`, `wchar`, `wctype` | 🖥️ Mac | 🟡 |
