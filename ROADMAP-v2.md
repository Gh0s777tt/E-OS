...
| 8 | **`R-612a`** ✅ | **Odwrócenie kolejności zapisu:** ESP i bootloader dopiero **po** zapisaniu roota; przerwana instalacja nie zostawia dysku, który **wygląda** na bootowalny | **ZROBIONE** ([eos-installer !3](https://gitlab.com/e-os/eos-installer/-/merge_requests/3)) | zmierzone w obie strony, zabicie instalatora w tym samym miejscu (`timeout -s KILL 5`, ostatni etap „Installing to RedoxFS”), z inspekcją **obszaru ESP na 1 MiB**, nie całego obrazu: stary kod → `FAT12, 1× BOOTX64`; nowy → `brak systemu plików, 0× BOOTX64`; instalacja dokończona → `FAT12, 1× BOOTX64` | `[P0·S·🖥️]` |
...
**Zadania 1, 7 i 8 są ✅ od 2026-08-30 — po scaleniu i podbiciu przypięć.** Wcześniejsza
wersja tej sekcji tłumaczyła, dlaczego mają 🚧: kod żył w niescalonych MR-ach, a
`recipes/core/installer/recipe.toml` przypinał commit sprzed poprawek. Oba warunki są
spełnione: MR-y scalone (e-os !1–!7, eos-installer !1–!4, eos-pkgutils !1–!3),
przypięcia podbite (`eos-installer` → `74726c889b`, `eos-pkgutils` → `ec08f22aa6`),
`scripts/eos-repos.sh pins --strict` → **26 OK, 0 dryfu**, `scripts/verify.sh` na `main` →
**15 PASS, 0 FAIL, 0 SKIPPED**.

Do pełnego domknięcia M1 brakuje zadania 11 — pierwszego przebiegu na fizycznym pececie.
Ono jedyne rozstrzyga kryterium akceptacji kamienia i wymaga sprzętu.

**Historyczna nota, zostawiona celowo.** Kod jest napisany i zmierzony, ale ✅ w tym
dokumencie znaczy „jest w produkcie", a nie jest:

- Zadanie 1 (`R-611a`) żyje na gałęzi `feat/m1-bootable-medium` — **niescalonej**.
- Zadania 7 i 8 (`R-607a`, `R-612a`) żyją w forku `eos-installer`, MR !2 i !3 — **niescalonych**.
  Nawet po ich scaleniu produkt ich nie dostanie, dopóki nie zmieni się przypięcie:
  `recipes/core/installer/recipe.toml:5` i `repos.toml:116` nadal wskazują
  `rev = c8d32ad39e5c778fb8aec51c953f2898d9e55495`, czyli commit sprzed tych poprawek.
  `scripts/eos-repos.sh pins --strict` potwierdza: `eos-installer | master | c8d32ad39e | OK(tip)`.

Do ✅ brakuje więc trzech rzeczy: scalenia MR-ów w forku, podbicia `rev` w recepturze
i `repos.toml`, oraz scalenia MR-a w repozytorium głównym. Pierwotne ✅ było przedwczesne —
znalezione przy audycie dryfu, w sekcji A.5.

**Sprostowanie do zadania 8, wpisane po wykonaniu.** Pierwotny opis miał dwa błędy, oba
wychwycone dopiero przy implementacji:

1. **Cytat nie popierał tezy.** Zdanie *„stage 2 then found an unbootable disk"* z
   `scripts/install-smoke-drive.py:87` opisuje **błąd pierwszej wersji harnessu** (dopasowywał
   prompt sprzed instalacji, więc wracał natychmiast i zabijał maszynę w połowie zapisu), a nie
   kolejność zapisu w instalatorze. Teza była prawdziwa — kolejność sprawdzono w kodzie
   (`with_whole_disk`: GPT → ESP → RedoxFS) — ale dowód był podstawiony.
2. **Obiecany skutek jest nieosiągalny.** „Przerwanie zostawia dysk, który **nadal bootuje stary
   system**" nie może zajść przy żadnej kolejności tych zapisów: GPT musi powstać, zanim
   cokolwiek trafi do partycji, więc stara tablica partycji ginie na długo przed ESP, a nowy root
   nadpisuje stare dane. Osiągalna i zaimplementowana własność jest inna, ale realna:
   **„oczywiście niezainstalowany" zamiast „wygląda na zainstalowany i nie jest"** — firmware nie
   znajduje wpisu rozruchowego i przechodzi do następnego nośnika, czyli do pendrive'a, z którego
   instalację uruchomiono.
