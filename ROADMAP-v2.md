...
| **V2-MS12** 🔑 | **Klucz podpisujący pakiety musi stać się kluczem operatora** — dziś cookbook **generuje go sam** (`src/cook/package.rs:42-51`: brak `build/id_ed25519.toml` → `SecretKeyFile::new()`) | odkryte przy V2-MS02. To jest klucz, którym podpisano **78 opublikowanych pakietów** i który klienci przypinają. Klucz wytworzony automatycznie przez narzędzie budujące nie jest kotwicą zaufania — jest artefaktem builda. Zmiana unieważnia opublikowane artefakty, więc to **decyzja operatora**, nie refaktor | 🔑 operator | 🔴 **[P1]** |
...
|---|---|---|---|---|
| **V2-STD01** 🖥️ | **POSIX (ISO/IEC 9945)** — zmierzyć `relibc` zestawami `os-test` (Sortix) i Open POSIX Test Suite | **największa dźwignia na całej liście.** Desktop potrzebuje portowanego oprogramowania, a porty rozbijają się o brakujące funkcje libc. Dziś nie ma **żadnej liczby** mówiącej, ile POSIX-a E-OS ma | **S** — zbudować obraz `os-test`, odpalić pod QEMU, opublikować wynik | 🔴 |
