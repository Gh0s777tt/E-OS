...
- **Jedno getty na konsolę szeregową** (#24) — konfiguracja x86_64 startowała getty na konsoli jądra w **dwóch** miejscach naraz: `nowait getty /scheme/debug/no-preserve -J` w `30_console` oraz `30_serial-getty.service` z `args = ["/scheme/debug"]`. Dwa getty to dwa procesy `login` czytające to samo wejście: po zalogowaniu pierwszy oddawał TTY shellowi, a drugi nadal czytał — i połykał kolejną wpisaną linię, odpowiadając *„Login incorrect"*. Widać to było w logu jako baner `issue` wypisany **dwa razy** (`login` wypisuje go raz na instancję) i dwa prompty `eos login:` odmalowane obok siebie. Zmierzone: z duplikatem `ci-install-smoke.sh` na x86_64 przechodził **2 razy na 7**, bez niego **5 na 5**. Przy okazji sprostowany komentarz twierdzący, że wejście po szeregowym nie dociera do gościa pod QEMU na macOS — cały harness steruje gościem właśnie tą drogą.
