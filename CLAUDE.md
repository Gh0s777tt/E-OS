...
| P-32 | **Weryfikacja binarki w obrazie: `build/fstools/bin/` ma tylko `redox_installer*`, a `redoxfs` przyjmuje pierwszy blok partycji szesnastkowo** — montowanie kopii partycji REDOX z kontenera `redox-base` padło cztery razy z rzędu (2026-09-05): `build/fstools/bin/redoxfs` nie istnieje, `redoxfs kopia /mnt/eos 512` (dziesiętnie) kończy się `not able to mount path`, `fdisk` w obrazie kontenera nie ma, a skrypt uruchomiony `bash /tmp/x.sh` (bez `-l`) nie ma `/usr/sbin` w PATH, więc `fdisk` milczy i offset wychodzi pusty | `cmp`/`sha256sum` na `/mnt/eos/usr/bin/<app>` melduje „No such file" — dowód „binarka w obrazie = `stage/`" nie powstaje, choć `cook` i `eos-build.sh` są zielone | `podman run --rm --cap-add SYS_ADMIN --device /dev/fuse -v eos-work:/work … bash -l skrypt`; kopia `build/x86_64/eos/harddrive.img` → `/work/mount-copy.img`; offset z GPT (Python `struct`: nagłówek pod LBA 1, wpisy pod LBA 2; partycja REDOX zaczyna się od LBA 4096), `build/build/fstools-target/release/redoxfs /work/mount-copy.img /mnt/eos 200` (4096/8 = 512 = `0x200`) montuje w ~1 s; potem `cmp` ze `stage/usr/bin/<app>`, `strings -c`, `fusermount -u`, kopię usuń. Zmierzone przy `e-os!133`: `/usr/bin/eos-guard` `6353dcdc…` identyczny ze `stage/` |
