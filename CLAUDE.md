...
| dokumentacja (`*.md`, `docs/SUMMARY.md`) | kontrola 14 (`eos-check-doc-paths.py`), kotwice `](#...)` istnieją, **brak zdublowanych numerów podsekcji**, CRLF zachowane tam, gdzie przypięte (`CHANGELOG.md`), `lychee --offline` gdy jest, `mdbook build` gdy zmienia się `SUMMARY.md`; **ROADMAP:** kontrola 16 (`eos-check-roadmap.py`) sprawdza kotwice `](#…)`, brak zdublowanych numerów nagłówków, **jeden status na identyfikator** (reguła 3: defekt tylko, gdy ten sam ID ma dwa RÓŻNE znaczniki; wiersze bez znacznika i ID spoza `ID_RE` — `V2-MS*`, `V2-STD*`, `ADR-*` — pomija) i ✅ tylko z dowodem `U-NNN`/MR/#issue w tym samym wierszu; **unikalność nowego ID w rodzinie (Annex A) nie ma bramki** — drugi wiersz z tym samym ID i tym samym statusem przechodzi (zmierzone 2026-09-04/05: zdublowany wiersz `RH-018` ✅ → `298 identifiers, 0 defects`, `ci-integrity.sh` PASS; `R-F26` ma dziś dwa wiersze ✅, `ROADMAP.md:868` i `:1619`), więc przed dopisaniem sprawdź `grep -n '<ID>' ROADMAP.md` — ID może legalnie wracać tylko w tabelach bez statusu (Annex B/C, ROADMAP §18.4) | `python3 scripts/eos-check-doc-paths.py && bash scripts/ci-integrity.sh` + `scripts/eos-check-roadmap.py` (kotwice, zdublowane numery nagłówków, jeden status na ID, ✅ z dowodem — **nie** unikalność ID) | wstaw ścieżkę do nieistniejącego pliku → kontrola 14 czerwona; zdubluj `### 6.3` → skrypt czerwony; zdubluj wiersz ID ze **zmienionym** statusem → `FAIL: … has 2 statuses across rows …`; z tym samym statusem → zielony (bramki nie ma) |
...
| `CLAUDE.md` | każda cytowana strona `docs/…md` istnieje (kontrola 14 — regex w `eos-check-doc-paths.py:83` widzi **tylko** ścieżki `docs/…md`) i każde cytowane `§N` prowadzi do sekcji (kontrola 21, `eos-check-claude-refs.py`); **istnienie nazwanych `scripts/*` nie ma bramki** — żadna kontrola `ci-integrity.sh` nie czyta ścieżek `scripts/` z `CLAUDE.md`, a skasowanie skryptu, który `ci-integrity.sh` sam wykonuje, czerwieni wywołującą go kontrolę (np. 16 dla `eos-check-roadmap.py`) z mylącym komunikatem — kontrolę 14 tylko wtedy, gdy skasujesz jej własny `eos-check-doc-paths.py`; sprawdzaj ręcznie `ls` na każdej nazwanej ścieżce, a przy zmianie numeracji `grep -n 'CLAUDE.md §'` w repo | jak dokumentacja | wstaw ścieżkę `docs/<nie-ma>.md` → kontrola 14 czerwona; usunięcie nazwanego skryptu, którego żadna bramka nie wykonuje, **nie** czerwieni niczego (zmierzone 2026-09-04/05: `git rm scripts/eos-rebase-check.sh` — nazwany w `CLAUDE.md:512` — → kontrola 14 zielona, `ci-integrity.sh` PASS); skrypt, który `ci-integrity.sh` sam wykonuje, czerwieni swoją kontrolę z mylącym komunikatem, nigdy 14 |
...
| P-22 | **`make r.<receptura>` nie dotyka `repo.tag`** — cel `r.%` (`mk/repo.mk:103`) jest celem `FORCE`, który tylko gotuje; `$(REPO_TAG)` (`mk/repo.mk:3`) zależy od `prefix`, konfiguracji i fstools, **nie** od wyników receptur, a `harddrive.img` i medium instalacyjne (`mk/disk.mk:3,20`) zależą od `repo.tag`. Po ręcznym cooku `make all` uznaje obraz za aktualny | zmierzone 2026-09-04 (`!119`): `make r.<r>` ×6 ugotowało wszystko poprawnie, a `eos-build.sh` zameldował „make produced NOTHING” (`scripts/eos-build.sh:123`) i nie wyeksportował nic nowego — to był czwarty z pięciu przebiegów, zanim obraz z nowymi pinami powstał | po ręcznym cooku usuń w wolumenie trzy produkty składania: `build/<arch>/eos/{repo.tag,harddrive.img,<medium>.img}` (to samo robi cel `rebuild`, `Makefile:43`), potem `eos-build.sh <arch>`. **Poprawna kolejność po bumpie pina:** P-20 (`rm -rf recipes/<grupa>/<r>/{source,target}`) → `make r.<r>` → usuń trzy produkty → `eos-build.sh <arch>` → kontrola „ugotowane == przypięte” (`eos-check-source-revs.py`, w `eos-build.sh`) → `ci-install-smoke.sh` |
| P-23 | **`cargo llvm-cov` odrzuca `--workspace` przy podkomendzie `run`, a `report` bez `--workspace` liczy tylko pakiet bieżący** — `--workspace` jest dozwolone tylko dla `[test,nextest,nextest-archive,show-env,clean,report,no subcommand]` (statyczna kontrola argumentów, `cli.rs:1389`; `run` przyjmuje `-p`), a `report` bez `--workspace`/`-p` wybiera pakiet, do którego prowadzi manifest z cwd (`context.rs:346-352`), i wycina źródła pozostałych członków domyślnym `ignore-filename-regex`, choć ich dane leżą w tych samych `.profraw` | zmierzone 2026-09-04 (`cargo-llvm-cov 0.9.0`, `cargo 1.98.0`, `~/eos-guard-work` @ `8972181`): `cargo llvm-cov --no-report --locked --workspace run -- --selftest` → `error: --workspace is specific to […] and not supported for subcommand 'run'`, exit 1 po ~16 ms, bez tworzenia `CARGO_TARGET_DIR`; identyczny błąd zakończył job 16317716478 (`checks-heavy`, `eos-guard!6` @ `b447f3f9`, `.gitlab-ci.yml:285`) „ERROR: Job failed: exit status 1”. Po `--no-report --locked --workspace` (25 testów `eos-fsintegrity` + 1 `eos-guard`) i `--no-report --locked run --no-default-features -- --selftest` ten sam zestaw 57 `.profraw` daje: `report --summary-only` → 4 pliki, 312 linii / 175 niepokrytych / **43,91 %**; `report --summary-only --workspace` → 7 plików, 952 / 216 / **77,31 %**; `report -p eos-fsintegrity` → 640 / 41 / 93,59 %. Sekwencja CI z domyślnymi cechami (lokalne powtórzenie): 311/174 → **44,05 %** bez `--workspace`, 951/215 → **77,39 %** z nim — tyle stoi w komentarzu `.gitlab-ci.yml:294-295`, a job 16317937966 na `main` drukuje 77,39 %; różnica 1 linii `main.rs` wynika z cech budowania przebiegu selftestu | `--workspace` na przebiegu **testowym** i na `report`, nigdy na `run`; pokrycie członka widać też przez `report -p <członek>` albo `report` z cwd w katalogu członka. `main` `8972181` ma to już w `.gitlab-ci.yml:288,297` |
| P-24 | **Trzeci argument `redoxfs` (`[block in hex]`) to numer bloku 4 KiB w zapisie szesnastkowym, bez prefiksu `0x`** — `mount.rs:365` `u64::from_str_radix(opts, 16)`: dziesiętne `512` czyta się jako `0x512` = blok 1298, a `0x200` odpada już na parsowaniu argumentów (`print_err_exit`, `mount.rs:120-124`); podany blok musi zawierać ważny nagłówek z 256-blokowego pierścienia, bo `FileSystem::open` sprawdza wtedy **tylko** ten blok (`block_opt.map_or(0..65536, \|x\| x..x + 1)`), a bez argumentu skanuje bloki 0..65536 | zmierzone 2026-09-05 00:09 (22:09Z) na **kopii** `harddrive.img` (P-6; fstools `redoxfs` z `58824d70`; nagłówek RedoxFS pod bajtem 2097152 = blok `0x200` = sektor 4096, trzecia partycja GPT — indeks 2, nazwa `REDOX`): `redoxfs <kopia>.img /mnt/a 512` → `failed to open filesystem …: No such file or directory` + `not able to mount path …`, exit 1 (ENOENT z `FileSystem::open`, nie brak pliku); `… 200` → montuje (`ls /mnt/a/usr/bin`: `[`, `audiod`, `autopoint`), exit 0; `… 0x200` → `invalid block '0x200': invalid digit found in string` + linia użycia, exit 1, bez próby montowania; bez argumentu → montuje, exit 0. Wszystkie 256 bloków pierścienia `200..2FF` mają ważny nagłówek, więc `201`, `2FF` i `+200` montują tak samo, a `1FF` i `300` padają jak `512` (zmierzone w trybie demona; `512`, `200`, `0X200`, `1ff` powtórzone w `-d` z identycznymi kodami wyjścia) | podawaj numer bloku 4 KiB gołym hexem (`200`) albo pomiń trzeci argument, gdy pierścień leży w pierwszych 256 MiB; `mk/disk.mk:56,71,81,91` i `scripts/mount-redoxfs.sh:115` nie przekazują go wcale |
| P-25 | **RedoxFS zapisuje rekordy plików (128 KiB) skompresowane LZ4, gdy się opłaca, więc szukanie fragmentów binarki `dd`/`grep` w surowym `harddrive.img` daje fałszywe negatywy i fałszywe pozytywy** — `transaction.rs:1912-1940` (redoxfs 0.9.1 `58824d70`; identyczny kod w upstreamowym 0.8.0/0.9.1, bez cechy ani przełącznika) zapisuje rekord jako 2 B długości + blok `lz4_flex`, gdy `comp_len+2 <= 65536`, inaczej surowo pod blokiem wyrównanym do 4 KiB; odczyt dekompresuje (`transaction.rs:407-425`). Decyduje ściśliwość rekordu, nie offset w pliku | zmierzone 2026-09-04/05 na `eos-control` (16 891 424 B, sha256 `f9ac179b…`): 27 ze 129 rekordów leży skompresowanych (0, 6-10, 31-32, 72-73, 108, 110-113, 117-128 — nagłówek ELF, kawałki `.text`, brzegi `.rodata`, `.eh_frame`, całe `.data.rel.ro`), 102 surowo; ponowne przepuszczenie rekordów przez `lz4_flex 0.11.6` odtwarza tę decyzję i każdy `comp_len` co do bajta. Fragment 4 KiB z `skip=4`, `256`, `1024` → 0 trafień, `skip=2048` → 1, `skip=3000` → 7 (jedno własne + sześć kopii tych samych bajtów w `eos-drive/guard/notes/sheets/slides/store`); fragmenty rekordów surowych trafiają się zawsze (100/100), rekordów skompresowanych nigdy pod własnym adresem wyrównanym do 4 KiB (0/27; pierwsze fragmenty rekordów 72 i 110 trafiają się przypadkiem, nie wyrównane — 72 w bajtach `eos-notes`, 110 w ciągu literałów własnego bloku LZ4). Krótkie łańcuchy nie ratują: okna 32 B z rekordów skompresowanych 91/788 wobec 247/247 z surowych. Kopia partycji zamontowana `fstools/bin/redoxfs` oddaje plik bajt w bajt (`cmp` exit 0, ta sama sha256) | artefakt sprawdzaj na zamontowanej **kopii** (P-6): `cmp`/`sha256sum` pliku z `/mnt/<kopia>/usr/bin/<plik>` wobec binarki stage. Surowy `grep` mówi coś tylko o rekordach, które `lz4_flex` zostawia surowe (`comp_len+2 > 65536`), przy wyrównaniu 4 KiB i pełnym dopasowaniu 128 KiB — czyli po przeliczeniu, nie z pamięci |
| P-26 | **`NIE SPRAWDZONO` nie odróżnia `eos-control` od `eos-guard`, a `grep -a` po surowym obrazie nie liczy wystąpień w binarce** — łańcuch to format `scope_note` wspólnej skrzynki `eos-fsintegrity` (`crates/eos-fsintegrity/src/db.rs:170` w repo Guarda; `eos-control` ciągnie ją przez git, `Cargo.toml:34`) plus po jednym asercie w `selftest.rs` każdego produktu, więc ma go każdy konsument silnika; liczba trafień w obrazie zależy od tego, które rekordy RedoxFS zostały skompresowane (P-25) | zmierzone 2026-09-04/05: binarki stage `eos-guard` (`658d84e2…`) i `eos-control` (`f9ac179b…`) mają po 2 wystąpienia, surowy `harddrive.img` (22:21) pokazuje 2 (nie 4), a `target.img` (19:14) — 1, bo jego `/usr/bin/eos-control` (`fe20b895…`, z zamontowanej kopii) to **starsza** budowa bez tego łańcucha (jego `eos-guard`, `d2e07dc9…`, też jest starszy), a trafienie pochodzi z `eos-guard` — drugie, UI-owe wystąpienie Guarda leży tam zniekształcone przez LZ4 pod bajtem 319394574. Surowy `grep` „potwierdził” więc łańcuch w obrazie, w którym `eos-control` go nie ma. Łańcuchy specyficzne dla nowej wersji (`no REMOVED finding for a file that really went missing`, `eos-fsintegrity`) są w nowym `eos-control` 1×, a w surowym `harddrive.img` 0× | produkt rozpoznawaj po `GUARD-SELFTEST-OK` / `EOS-CONTROL-SELFTEST-OK` (po jednym w `/usr/bin/eos-guard` i `/usr/bin/eos-control`, w żadnym innym pliku obu obrazów) — to identyfikuje produkt, nie budowę (stary `eos-control` też go ma). Że **nowa** binarka trafiła do obrazu, dowodzi tylko sha256 pliku z zamontowanej kopii (P-6, P-25) |
| P-27 | **`cargo metadata --no-deps` wypisuje `packages` z członkami workspace'u przed pakietem głównym, więc `['packages'][0]` po dodaniu członka wskazuje silnik, nie produkt** — cargo 1.98.0 (także nightly 1.100.0) emituje wpisy z `members` (kolejność listy; globy posortowane ścieżką), a manifest główny dopisuje na końcu (chyba że `.` stoi w `members` — wtedy według listy); `default-members` nie zmienia kolejności, cwd też nie, a `workspace_default_members` zwraca id pakietu głównego | zmierzone 2026-09-04, powtórzone 2026-09-05, w `~/eos-guard-work` @ `89721810` (`[workspace] members = ["crates/eos-fsintegrity"]`): `['eos-fsintegrity', 'eos-guard']`, `workspace_default_members` = `path+file:///…/eos-guard-work#eos-guard@0.2.0`. `packaging/release.sh` w `eos-control`, `eos-notes`, `eos-sheets`, `eos-slides`, `eos-drive` i `eos-store` (ref `main`, 8721 B, sha256 `42d003ef…`; `meta()` w linii 132 czyta `['packages'][0]`) jest dziś poprawny tylko dlatego, że żaden z tych sześciu `Cargo.toml` nie ma `[workspace]`; kopia Guarda (8991 B, `bf104edd…`, linie 132-134) wybiera pakiet o `manifest_path == workspace_root/Cargo.toml` i różni się od tamtych tylko tym hunkiem. Zdanie z `.gitlab-ci.yml` `eos-control` (linie 53-54) „the same file, byte for byte, in every E-OS product repository” jest nieprawdziwe dla jednego z siedmiu, a CI Guarda dalej nazywa plik „the shared template” (linia 205) | w skryptach wydania wybieraj pakiet po `manifest_path == workspace_root/Cargo.toml` (jak `release.sh` Guarda), nie po `[0]`; dodając workspace do kolejnego produktu przenieś tę poprawkę razem z nim i popraw komentarze o „byte for byte” |
| P-28 | **Hook `commit-msg` lefthooka odrzuca domyślny temat scalenia, więc `git merge --no-edit` (gdy nie jest fast-forwardem) zatrzymuje się z `MERGE_HEAD`, a następny `git commit -m 'docs: x'` staje się commitem scalającym o tym temacie** — `lefthook.yml:39-45` przyjmuje tylko tematy Conventional Commits, a `Merge branch '…'` i `Merge remote-tracking branch 'origin/main' into …` odpadają (`grep` exit 1); git melduje „Not committing merge; use 'git commit' to complete the merge”, zostawia `MERGE_HEAD` i w pełni przygotowany indeks. Fast-forward nie tworzy commitu i hooka nie woła; `merge.commit`/`merge.ff`/`core.editor`/`core.hooksPath` są nieustawione | zmierzone 2026-09-04 (git 2.55.0, lefthook 2.1.12; plik hooka z 2026-09-03 13:42:57, reguła w `lefthook.yml` od `f45cf1fdb`, 2026-07-16): w meta-repo `git merge --no-edit origin/main` na czystym drzewie (rodzice scalają się czysto, `git merge-tree --write-tree` exit 0) wypisał `Nie składanie scalenia; użyj „git commit”, aby ukończyć scalanie.`, a `git add CHANGELOG.md && git commit -F -` dał dwurodzicielski `e13f1ec87` (22:04) z tematem `docs(changelog): …`. W repozytorium jednorazowym z tą samą sekcją hooka: `git merge --no-edit other` → `commit must be Conventional Commits`, exit 1, `MERGE_HEAD` obecny, `git status --short` = `A  b.txt`; `git commit -m 'docs: x'` → commit o 2 rodzicach. Drugi wpis `commit (merge)` w reflogu, `637b1aeb0` (18:51), to konflikt treści w `CHANGELOG.md` — tam hook tylko wymusił konwencjonalny temat commitu rozwiązującego | scalaj z tematem konwencjonalnym: `git merge --no-edit -m 'chore(merge): …' origin/main` (exit 0, bez `MERGE_HEAD`); `LEFTHOOK=0`/`--no-verify` omijają hook, `skip: [merge]` w sekcji `commit-msg` zwalnia tylko scalenia. Po zatrzymaniu dokończ `git commit -m 'chore(merge): …'` — bez `-m` hook odrzuci domyślny temat ponownie, a w powłoce bez tty `git merge X` i tak działa jak `--no-edit` |
| P-29 | **`cmd 2>&1 \| tail -N` w sondzie gubi treść błędu, nie tylko status — `set -o pipefail` z P-3 tego nie naprawia** — `tail -6` zostawia sześć ostatnich linii, a po błędzie kompilacji `cargo`/cook drukują jeszcze podpowiedź `To reuse those artifacts…`, `redoxer cargo: exit status: 101`, `redoxer env: exit status: 1`, `cook … - failed`, `Failed to run […]`, `make: *** … Error 1`: sam błąd `rustc` jest już poza oknem. To mechanizm znaleziska `C-2` (§5.5; `eos-build.sh` nadal ma `\| tail -2` w `:66` i `\| tail -3` w `:92`), tylko w skryptach sond | zmierzone 2026-09-04: pierwszy cook `mkisofs-rs` (`s15.sh:18`: `… r.mkisofs-rs 2>&1 \| tail -6`, `rc=${PIPESTATUS[0]}`) zostawił w `s15.log:28-33` dokładnie 6 linii i `rc=2`, bez `E0635`/`failed to compile`; drugi cook (`s15b.sh:9`: `grep -aE 'error…' \| tail -25`, 19 linii) pokazał `error[E0635]: unknown feature proc_macro_span_shrink` w linii 14 z sześcioma liniami po nim — że ten sam błąd stał poza oknem pierwszego przebiegu, jest wnioskiem (surowy strumień nie przetrwał), nie pomiarem. `rc=2` w logu pochodzi z `${PIPESTATUS[0]}`, nie z `pipefail`: na hoście (bash 3.2.57) polecenie drukujące błąd + 10 linii, exit 101, przez `tail -6` daje 6 linii bez błędu, `pipefail` zmienia tylko `$?` (0 → 101), `${PIPESTATUS[0]}` = 101 z nim i bez; to samo w kontenerze (bash 5.2.37, GNU tail 9.7) z poleceniem o statusie 2: `$?` 0 → 2, `${PIPESTATUS[0]}` = 2 w obu wariantach, błąd przeżywa dopiero od `tail -9` | w sondach nie ucinaj: `2>&1 \| tee "$LOG" \| tail -6` zachowuje pełny strumień i `${PIPESTATUS[0]}`, albo filtruj `grep -aE` po `error` zamiast `tail`; log w całości do `dowody/`. `pipefail` zostaw dla statusu (P-3), ale nie licz na niego przy diagnozie |
| P-30 | **Heredoc Pythona z `assert` przed `open(p,'w').write(s)`, po którym w NASTĘPNEJ linii stoi `git add … && git commit -m 'docs: …'`, robi commit „o zmianie”, której nie ma** — nieudany `assert` zostawia plik bajt w bajt (zapis nieosiągnięty, exit 1), ale nowa linia to separator sekwencyjny, nie `&&`: `git add && git commit` idzie dalej, a narzędzie Bash sesji (zsh 5.9) melduje status tylko ostatniego polecenia. Commit powstaje zawsze, gdy po `git add` indeks różni się od HEAD — wcześniejsza niezacommitowana zmiana tego pliku, plik nieśledzony, cokolwiek innego już w indeksie, `-a` z obcą zmianą | zmierzone 2026-09-04 (Python 3.14.7, bash 3.2.57, zsh 5.9, git 2.55.0, repo jednorazowe): `s=s.replace('DOES_NOT_EXIST','gamma = 3'); assert 'gamma' in s; open(p,'w').write(s)` → `AssertionError`, exit 1, sha256 `target.txt` bez zmiany; przy czystym drzewie `git commit` odmawia (`nothing to commit, working tree clean`, exit 1; z obcą niezaindeksowaną zmianą `no changes added to commit`), ale z wcześniejszą zmianą w pliku powstał `8ec441f` z tematem obiecującym `gamma` i 0 jego wystąpieniami, a przez narzędzie Bash sesji `9f930b8` bez zgłoszonego błędu. Prawdziwy łańcuch `python3 - <<EOF && git add … && git commit …` zatrzymuje się (exit 1, bez commitu), `set -e` też — ale nie gdy krok Pythona stoi w potoku (`2>&1 \| tee py.log`) bez `set -o pipefail` (commit `c48a162` powstał). `PYTHONOPTIMIZE=1`/`python3 -O` wycina `assert` (exit 0, plik przepisany tymi samymi bajtami), a `assert` wewnątrz `with open(p,'w')` obcina plik do 0 B | sprawdzaj wynik podmiany, nie status: po kroku `git diff --stat -- <plik>` i `grep -c '<nowy tekst>' <plik>` w tym samym poleceniu, dopiero potem `git add <plik>` (nie `-A`/`-a`) i `git commit`; jeśli łańcuch, to `&&` w linii heredoca albo `set -e -o pipefail`, nigdy w kolejnej linii |
| P-31 | **`install-smoke-drive.py` naciskał Enter „na ślepo” po stałych 12 s, a menu trybu wideo bootloadera czeka na klawisz bez limitu czasu** — `EOS_SMOKE_SLOW` skalował okna kroków, ale nie ten `time.sleep(12)`; klawisz wysłany przez monitor QEMU, zanim menu istnieje, ginie, a gość stoi na menu w ciszy do końca budżetu z żywym QEMU — dokładnie sygnatura przebiegu z 20:03 2026-09-04 (92 min w etapie 2, FAIL na budżecie), gdy równolegle kompilował runner `local-gates` | zmierzone 2026-09-05 na kopii zainstalowanego dysku z linią QEMU etapu 2 (`dowody/smoke-menu-fix-2026-09-05/`): stary sterownik bezczynnie → `login:` w 19 s; stary + 64 procesy `yes` → menu widoczne, `usr/lib/boot/kernel` nigdy, FAIL po budżecie 150 s; nowy + 64 `yes` → PASS w 57 s; nowy bezczynnie → 10 s; nowy z błędnym gniazdem monitora → trzy ponowienia i FAIL (harness dalej umie paść). Ścieżka produkcyjna `ci-install-smoke.sh … 5400 --arch x86_64`: bezczynnie PASS 83 s, pod 64 `yes` PASS 6 min 22 s | Enter dopiero po markerze menu na serialu (`Arrow keys and enter select mode`, okno `w(60)`), potwierdzenie, że rozruch poszedł dalej (`usr/lib/boot/kernel`, okno `w(15)`), inaczej ponowienie do 3 razy; ślepe 12 s zostaje tylko jako zapasowa ścieżka dla bootloadera bez tekstu menu. Tak samo robił już `ci-boot-smoke.sh:116-140`. Wniosek ogólny: FAIL harnessu pod obciążeniem sprawdź na kopii dysku z hogami, zanim uznasz go za wadę obrazu |
| P-32 | **Weryfikacja binarki w obrazie: montaż kopii partycji REDOX z kontenera `redox-base` pada, gdy offset liczy `fdisk`, a blok podaje się dziesiętnie** — `fdisk` w obrazie kontenera nie ma (a skrypt uruchomiony `bash /tmp/x.sh` bez `-l` nie ma nawet `/usr/sbin` w PATH), więc offset z `fdisk -l` wychodzi pusty i `redoxfs` dostaje blok 0 (`failed to open filesystem … No such file or directory`, potem `not able to mount path`); trzeci argument `redoxfs` to pierwszy blok partycji **szesnastkowo** (`200` = LBA 4096/8) — dziesiętne `512` pada tak samo. Cztery nieudane próby z rzędu 2026-09-05. Sama binarka **istnieje**: `build/fstools/bin/redoxfs` i `build/build/fstools-target/release/redoxfs` są `cmp`-identyczne (pierwsza wersja tego wiersza, `!134`, twierdziła po `ls … \| head -5`, że jej nie ma — listing obcięty po `redox_installer*`; sceptyk handoffu obalił to tego samego dnia, poprawka `!135`) | `cmp`/`sha256sum` na `/mnt/eos/usr/bin/<app>` melduje „No such file" — dowód „binarka w obrazie = `stage/`" nie powstaje, choć `cook` i `eos-build.sh` są zielone | `podman run --rm --cap-add SYS_ADMIN --device /dev/fuse -v eos-work:/work … bash -l skrypt`; kopia `build/x86_64/eos/harddrive.img` → `/work/mount-copy.img`; offset z GPT (Python `struct`: nagłówek pod LBA 1, wpisy pod LBA 2; partycja REDOX zaczyna się od LBA 4096), `build/fstools/bin/redoxfs /work/mount-copy.img /mnt/eos 200` (4096/8 = 512 = `0x200`) montuje w ~1 s; potem `cmp` ze `stage/usr/bin/<app>`, `strings -c`, `fusermount -u`, kopię usuń. Zmierzone przy `e-os!133`: `/usr/bin/eos-guard` `6353dcdc…` identyczny ze `stage/` |
| P-33 | **Dowód on-image okna GUI: QMP `input-send-event` **z identyfikatorem urządzenia** ubija QEMU 11.0.2 pod `-display none`; bez identyfikatora działa** — `{"execute":"input-send-event","arguments":{"device":"kbd0",…}}` kończy się `Unexpected error in object_property_find_err() … Property 'qemu-fixed-text-console.device' not found` (Abort trap 6) i maszyna znika razem z gościem, natomiast ta sama komenda **bez** pola `device` odpowiada `{"return": {}}` dla `rel`, `btn` i `key` (pierwsza wersja tego wiersza, `!134`, zabraniała `input-send-event` w całości — za szeroko; sonda bezdyskowa z 2026-09-05 to obaliła) | harness kończy się bez PNG; przyczyna tylko w `qemu.log`, serial urywa się bez słowa | klawiatura: QMP `send-key` (jeden klawisz na znak, `ret`, `tab`) — greeter Orbitala tak przyjmuje login, także gdy w maszynie jest `usb-tablet`; `screendump` z `"format":"png"` na `-vga std`. **Mysz jest zawodna:** względna mysz PS/2 (`query-mice` → `QEMU PS/2 Mouse`, `ps2d` żywy) **nigdy** nie ruszyła kursora Orbitala w czterech przebiegach, a `-device usb-tablet` (bezwzględny) trafił kursorem 1 raz na 3 przy identycznej linii poleceń — więc okno aplikacji otwieraj z powłoki roota na serialu (`eos-guard &` → okno nad greeterem, a po zalogowaniu nad pulpitem), a klikanie w ikonę traktuj jako opcję, nie jako ścieżkę pomiaru. **Nigdy nie wpisuj `dmesg`** (ani niczego, co czyta `/scheme/sys/log`) do powłoki roota na serialu tego gościa: nie wraca, `^C` nie ratuje, zostaje tylko QMP `quit`. Stderr sesji Orbitala i notki zadań iona lądują na serialu **po** prompcie, więc parser musi się resynchronizować. Komplet: `dowody/pr004-onimage-2026-09-05/{onimage.sh,onimage-drive.py,screenshot.sh,screenshot-drive.py,serial-selftest.sh,png-check.py}` (zmierzone 2026-09-05 na klonach zainstalowanego dysku; okno Guarda z trzema liniami zaufania w `new-disk-display/04-run.png`) |
| P-34 | **Gołe `cargo` w skrypcie receptury przepisuje `Cargo.lock` meta-repo, a nie locka receptury** — skrypt `[build]` startuje z katalogiem roboczym `${COOKBOOK_BUILD}` (`src/cook/cook_build.rs:421` `command.current_dir(&cookbook_build)`), który przy szablonie `custom` jest **pusty**; `cargo` nie znajduje tam `Cargo.toml`, więc idzie w górę drzewa i trafia na `/work/redox/Cargo.toml` — workspace cookbooka | polecenie melduje sukces na zupełnie innym pakiecie, kompilacja bierze starą wersję z `source/Cargo.lock`, a w drzewie budowania cicho zmienia się `Cargo.lock` meta-repo (`eos-sync-buildtree.sh --apply` przywróci go dopiero przy następnej budowie) | zmierzone 2026-09-05 na `recipes/wip/tools/mkisofs-rs` (§15 wiersz 33): `cargo update -p proc-macro2` w skrypcie wypisało `Updating proc-macro2 v1.0.103 -> v1.0.107` (wersje z `/work/redox/Cargo.lock`, w repo `1.0.103`), po czym `cookbook_cargo` skompilował `proc-macro2 v1.0.46` z `recipes/wip/tools/mkisofs-rs/source/Cargo.lock` i padł na `error[E0635]`; `find …/target -name Cargo.lock` nie znalazł żadnego locka w katalogu budowania. Obejście: adresuj manifest jawnie (`cargo update --manifest-path "${COOKBOOK_SOURCE}/Cargo.toml" -p <pakiet>`) albo dostarcz poprawiony `Cargo.lock` łatką receptury; po każdej takiej próbie sprawdź `grep -A1 'name = "<pakiet>"' /work/redox/Cargo.lock` i przywróć plik z repo (`podman run --rm -i -v eos-work:/work … 'cat > /work/redox/Cargo.lock' < Cargo.lock` — bind-mount z exFAT nie działa, P-5) |
| P-35 | **Prompt iona na serialu jest kolorowany i poprzedzony sekwencją tytułu, więc `root:` nie występuje w bajtach dosłownie** — sterownik dopasowujący prompt do **surowego** bufora czeka do końca okna nad gościem, który stoi bezczynnie przy prompcie: bajty to `\x1b]0;root: /home/root\x07\x1b[1;38;5;85mroot\x1b[39m:\x1b[38;5;75m~\x1b[39m# \x1b[0m` (tytuł OSC zawsze przed promptem; między `root` a `:` siedzi zmiana koloru, a po `# ` reset SGR) | przebieg wygląda na zawieszony na jednym poleceniu (`[window 1260 s]`), choć transfer się skończył i serwer HTTP odnotował `200`; dwa przebiegi po ~20 min do kosza (`row30` run 4 i 6, 2026-09-05, **nie dołączone do dowodów** — logi skasowane przy sprzątaniu klonów) | **lekarstwem jest samo zdjęcie sekwencji, nie zmiana wzorca**: `ANSI = re.compile(r"\x1b\][^\x07]*\x07\|\x1b\[[0-9;?]*[a-zA-Z]")`, potem `PROMPT_END.search(ANSI.sub("", buf[marker:]))` — sceptyk zmierzył, że **stary** wzorzec `(?:root:[^\n]*# \|:~# )[ \t]*$` na buforze po zdjęciu ANSI też zwraca `True` na wszystkich promptach w `run10`/`run11`; tolerancja na końcowe sekwencje, którą przy okazji dopisałem, jest nadmiarowa. Na surowych bajtach: stary wzorzec `False`, po zdjęciu `True`; poprawiony sterownik przeszedł ten sam scenariusz w 5 min (`dowody/s15-row30-2026-09-05/serial-selftest-driver.py`, `runs/run{8,10,11,12}-serial.log`) |
| P-36 | **W gościu `/tmp` to świeży system plików w RAM przy każdym rozruchu, a domem roota jest `/home/root`, nie `/root`** | zmierzone 2026-09-05 na tym samym dysku zaraz po przebiegu, który zostawił w `/tmp` 1 MiB danych (`dowody/s15-row30-2026-09-05/runs/p36-evidence-serial.log`): `ls -la /tmp` → tylko `.` i `..`; `echo $HOME` → `/home/root`; `curl -o /root/probe-p36.bin …` → `Warning: Failed to open the file /root/probe-p36.bin: No such file or directory` i `curl: (23) client returned ERROR on write of 16384 bytes`, a `ls -la /root/probe-p36.bin` → `No such file or directory` — **transfer HTTP przy tym działa**: to samo pobranie do `/tmp` kończy się plikiem 1 048 576 B | wszystko, co sonda potrzebuje, dostarczaj i uruchamiaj **w jednym rozruchu**; jeśli coś ma przeżyć restart, pisz do `/home/root` (istnieje, `drwx------ root root`, z `.config` i `.local`) |
| P-37 | **Budowa przerwana w pół (odpadnięcie wolumenu, `kill`) zostawia w `recipes/<grp>/<r>/target/` obcięty plik, a następna budowa pada z komunikatem o TOML-u, nie o przerwaniu** | zmierzone 2026-09-06: po odpadnięciu `/Volumes/Project itp` (§21.7) w trakcie `eos-build.sh x86_64` kolejny przebieg stanął na `missing field 'packages'` z `file: Some("recipes/core/base/target/x86_64-unknown-redox/auto_deps.toml")` i `make: *** [mk/repo.mk:7: build/x86_64/eos/repo.tag] Error 1`; sam plik był pusty. `P-20` każe czyścić `source` i `target` po **podbiciu pina** — to jest ten sam katalog, ale z innego powodu i bez zmiany pina | po każdym przerwanym `make` usuń `recipes/<grp>/<r>/target` (oraz `build/<arch>/eos/*.partial` i `repo.tag`) **zanim** zaczniesz szukać błędu kompilacji: komunikat wskazuje na receptury, a przyczyną jest przerwanie |
| P-38 | **Drzewo wniesione do kontenera przez `tar`/`podman cp` przychodzi ze STARYM mtime, więc cargo uznaje je za zbudowane i uruchamia stary plik binarny — czerwień lub zieleń, która niczego nie mierzy** | zmierzone 2026-09-07: poprawione `partitionlib` wniesione tarem do `/work/pl-x` zaraz po przebiegu drzewa *bez* poprawki dało `FAILED` w tym samym teście, który na hoście był zielony; źródła, obraz testowy i skrzynia `gpt` były md5-identyczne po obu stronach — jedyną różnicą był mtime plików, starszy niż obiekty z poprzedniego przebiegu, więc `cargo test` nie przebudował nic. Tydzień wcześniej to samo w drugą stronę zrobiłoby fałszywą zieleń. 2026-09-10 recenzent zmierzył wariant tej pułapki na drzewie współdzielonym z innymi agentami: `Finished in 0.34s` mimo `touch` i wynik z cudzego pliku | po każdym `tar xf`/`podman cp` do drzewa, w którym już budowano: `find <crate> -type f -exec touch {} +`, a na początku transkryptu `md5sum` każdego wejścia — ten wiersz md5 jest częścią dowodu, nie ozdobą. Agentom mierzącym równolegle daj **prywatną kopię** (`cp -r /work/pl-x /work/rev-<nazwa>`), nigdy wspólne drzewo |
| P-39 | **Budowa z niezacommitowanego podbicia pinu nazywa w logu i w indeksie repo commit-rodzica** — `eos-build.sh` synchronizuje drzewo robocze (P-21), ale nagłówek logu podaje `git rev-parse HEAD`, a `serial` indeksu (`EOS_REPO_SERIAL`) to `git rev-list --count HEAD` | zmierzone 2026-09-11: budowa pinów `eos-base` `e2a61358` / `eos-installer` `26dc944a` ruszyła o 03:33 na drzewie `4a1cf483c` + cztery zmienione pliki, commit `b02bf16cd` powstał dopiero o 04:10. Wejścia budowy okazały się bajt w bajt równe commitowi, ale log mówi „@ 4a1cf483c", a `repo/x86_64-unknown-redox/repo.toml` niesie `serial = 10692` — licznik rodzica (`b02bf16cd` = 10693, scalenie = 10694). Smoke i akceptacja w gościu dotyczyły tych samych obrazów, więc stanu, którego żaden commit nie nazywa | commit podbicia pinu (na gałęzi) **przed** `eos-build.sh`; po budowie sprawdź, że nagłówek logu i `serial` w `repo/<arch>/repo.toml` równają się `git rev-parse --short HEAD` / `git rev-list --count HEAD` |
| P-40 | **Nocna ciężka budowa ze schedule'u pracuje w tym samym kontenerze i wolumenie `eos-work` co sesja** — schedule „nightly heavy build (aarch64 image + boot-smoke)" (`0 3 * * *` Europe/Warsaw) uruchamia `build-image` na runnerze `eos-heavy` (ten Mac, executor shell), który robi `podman exec "$EOS_CONTAINER"` (`eosbuild`) `… make CI=1 ARCH=aarch64 … all` w `/work/redox` | zmierzone 2026-09-11: `build-image` (pipeline 2839329553, job 16436617442) trwało 03:12–03:38 CEST, `make` od 03:12:06. W wolumenie powstały wtedy `build/aarch64/eos/{harddrive,installer}.img`, `repo.tag`, `repo/aarch64-unknown-redox/*.pkgar` i `repo.toml` bez `serial`, a `build/fstools.tag` odświeżył się o 03:12 — 15 min po tym, jak sesja usunęła `recipes/core/{base,installer}/source` pod podbicie pinu (P-20); zadanie jeszcze trwało, gdy sesja gotowała (03:30) i budowała (03:33–03:36). W transkrypcie sesji nie ma po tym śladu: wyglądało to jak niewyjaśnione mtime, dopóki `glab api …/pipelines?updated_after=` nie wskazało zadania | przed pracą w wolumenie między ~02:50 a ~03:45 (Europe/Warsaw) sprawdź `glab api projects/e-os%2Fe-os/pipeline_schedules` i trwające zadania runnera `eos-heavy`; po nocnej budowie zweryfikuj rewizje źródeł na nowo (`eos-check-source-revs.py`, robi to `eos-build.sh`) i nie traktuj mtime w `build/`/`repo/` jako śladu własnej pracy. Czy nocna budowa ma dostać osobny wolumen — decyzja właściciela |
