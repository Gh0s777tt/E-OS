...
| P-35 | **Prompt iona na serialu jest kolorowany, więc `root:` nie występuje w bajtach dosłownie** — sterownik dopasowujący prompt do surowego bufora czeka do końca okna nad gościem, który **stoi bezczynnie przy prompcie**: bajty to `\x1b[1;38;5;85mroot\x1b[39m:\x1b[38;5;75m~\x1b[39m# \x1b[0m`, a po `curl` ostatnim znakiem jest reset SGR (`# \x1b[0m`), którego wzorzec „tylko spacje na końcu" nie przepuszcza | przebieg wygląda na zawieszony na jednym poleceniu (`[window 1260 s]`), choć transfer się skończył i serwer HTTP odnotował `200`; dwa przebiegi po ~20 min do kosza (`row30` run 4 i 6, 2026-09-05) | dopasowuj po zdjęciu sekwencji: `ANSI = re.compile(r"\x1b\][^\x07]*\x07\|\x1b\[[0-9;?]*[a-zA-Z]")`, potem `PROMPT_END.search(ANSI.sub("", buf[marker:]))`. Zmierzone na dokładnych bajtach zablokowanego promptu: stary wzorzec `False`, nowy `True`; poprawiony sterownik przeszedł ten sam scenariusz w 5 min (`dowody/s15-row30-2026-09-05/serial-selftest-driver.py`, `runs/run{8,10,11}-serial.log`) |
| P-36 | **W gościu `/tmp` to świeży system plików w RAM przy każdym rozruchu, a domem roota jest `/home/root`, nie `/root`** | sonda dwurozruchowa (pobierz pliki w pierwszym rozruchu, użyj w drugim) startuje na pustym katalogu: `ls -la /tmp` → tylko `.` i `..`, a `/tmp/imgd` z poprzedniego rozruchu „nie istnieje" (`command not found`); `curl -o /root/plik` kończy się `Warning: Failed to open the file /root/plik: No such file or directory` i `curl: (23) client returned ERROR on write` — plik nigdy nie powstaje, choć transfer HTTP **działa** | wszystko, co sonda potrzebuje, dostarczaj i uruchamiaj **w jednym rozruchu**; jeśli coś ma przeżyć restart, pisz do `/home/root`. Zmierzone 2026-09-05 (`row30` run 3 → run 5: pliki pobrane w run 3 zniknęły; `dowody/s15-row30-2026-09-05/runs/`) |
