...
| P-33 | **Dowód on-image okna GUI: QMP `input-send-event` ubija QEMU 11.0.2 pod `-display none`** — pierwsze takie polecenie kończy się `Property 'qemu-fixed-text-console.device' not found` (Abort trap 6) i maszyna znika razem z gościem | harness kończy się bez PNG; przyczyna tylko w `qemu.log`, serial urywa się bez słowa | klawiatura wyłącznie przez QMP `send-key` (jeden klawisz na znak, `ret`, `tab`) — greeter Orbitala przyjmuje tak login; `screendump` z `"format":"png"` na `-vga std`; okno aplikacji otwiera się z powłoki roota na serialu (`eos-guard &` → okno nad greeterem, a po zalogowaniu nad pulpitem), więc zrzut nie wymaga klikania w launcher. Komplet: `dowody/pr004-onimage-2026-09-05/{screenshot.sh,screenshot-drive.py,serial-selftest.sh}` (zmierzone 2026-09-05 na klonach zainstalowanego dysku; okno Guarda z trzema liniami zaufania w `new-disk-display/04-run.png`) |
